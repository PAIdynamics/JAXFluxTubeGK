...
linear objectives, and fixed-topology optimization helpers.  The production
...
The preferred current external W7-X reference is stella, because it is a CPU
continuum gyrokinetic code and can run the matched W7-X case locally.  The
prepared directory \path{fixtures/stella_w7x_mode_structure_run/} uses the GX
benchmark VMEC file, \(\texttt{torflux}=0.64\), \(\alpha=0\), adiabatic
electrons, \(k_y=(0,0.1,0.2,0.3)\), \(k_x=0\), \(\texttt{nzed}=256\),
\(\texttt{nmu}=8\), \(\texttt{nvgrid}=16\), \(\texttt{tend}=200\), and a
late-half-window growth convention.  The full matched run completed on one CPU
in about \(295\,\mathrm{s}\) and was exported to
\path{fixtures/w7x_itg_external_mode_structure_fixture.csv}.  The exported
stella values are
\[
\gamma=(-7.797962\times10^{-4},-4.876057\times10^{-3},9.619444\times10^{-3}),
\]
\[
\omega_r=(-6.573003\times10^{-2},-1.181846\times10^{-1},-2.849346\times10^{-3})
\]
for \(k_y=(0.1,0.2,0.3)\).

The solver can now import the stella \path{.geometry} table directly.  After
matching normalized \(z\), field-line length, selected \(k_y\), \(k_x=0\), and
late-time growth-window controls, the \(t=200\) comparison has good growth
agreement,
\[
\max_{k_y}|\gamma_{\rm solver}-\gamma_{\rm stella}|=8.00978267\times10^{-3},
\]
but the W7-X gate remains open.  The remaining discrepancy is concentrated in
\(k_y=0.3\): the closest stella-scale finite-difference velocity case has
\(\Delta\omega_r=-1.65161421\times10^{-1}\) and phase-aligned profile error
\(1.56517320\times10^{-1}\).  Frequency sign, affine rescaling, profile
conjugation, z reversal, circular shifts, longer time windows, and simple
velocity-grid refinement do not close the gate.

\subsection{Current W7-X blocker}

The active W7-X task is term-array parity for \(k_y=0.3\).  The solver-side
balance fixture \path{fixtures/w7x_ky03_rhs_model_balance/} reconstructs the RHS
with zero stored error and has quasineutrality residual near machine precision.
Its selected-mode RHS is streaming dominated:
\[
\begin{array}{c|c}
\hbox{term} & \|R_{\rm term}\|_2/\|R\|_2 \\
\hline
\hbox{parallel streaming} & 1.11324006\\
\hbox{magnetic drift} & 0.53073047\\
\hbox{mirror force} & 0.32597147\\
\hbox{parallel field drive} & 0.13985170
\end{array}
\]

Standard stella output is enough to verify normalized \(z\), \(F=b\cdot\nabla
z/(2\pi)\), \(B\), and \(k_\perp^2\) at the precision of the rounded
\path{.geometry} table, but it does not contain complex distribution or
per-term RHS/source arrays.  A non-destructive stella source-copy patch now
emits a selected-mode explicit-term trace for Fortran \texttt{iky=4},
\texttt{ikx=1}.  The raw trace is intentionally kept in \path{/tmp}; the compact
summary is stored in
\path{fixtures/w7x_ky03_stella_rhs_trace_summary/rhs_trace_summary.json}.

The first RHS-trace comparator converts stella-native
\(\texttt{rhs}\Delta t\) records to continuous-time RHS norms and compares
scale-free term fractions.  It exposed one concrete convention fix:
the stella-imported equilibrium-gradient drive coefficient must use the
stella global-header \texttt{flux\_fac}.  After this correction, the solver
and stella scalar fractions are
\[
\begin{array}{c|cc}
\hbox{group} & \hbox{stella} & \hbox{solver}\\
\hline
\hbox{parallel streaming bundle} & 0.89595108 & 1.16029482\\
\hbox{mirror force} & 0.29866891 & 0.24474080\\
\hbox{magnetic drift bundle} & 0.42804911 & 0.56430754\\
\hbox{equilibrium drive} & 0.77998161 & 0.61459833
\end{array}
\]
The old solver equilibrium-drive fraction was \(0.07783134\), so the
\texttt{flux\_fac} correction removed the dominant scalar error.  The remaining
blocker is a contract mismatch rather than a justified physics change: the
current stella trace has a duplicate z endpoint, \(N_{v_\parallel}=32\),
\(v_\parallel\in[-3,3]\), large Gauss-like \(\mu\) nodes, and no velocity
weights, while the committed solver balance fixture stores scalar summaries on
a smaller grid.  The next run should use the trace format with \texttt{wgts\_vpa}
and z-dependent \texttt{wgts\_mu}, then compare weighted complex arrays on a
shared or explicitly interpolated grid before changing the streaming/mirror
physics.

\subsection{Optimization status}

The optimization layer is useful for plumbing and reduced studies.  It exposes
single-surface objectives, scans over \(\rho\), \(\alpha\), and \(k_y\),
selected-growth and quasilinear proxy objectives, benchmark-target least-square
wrappers, and examples that differentiate through a fixed number of RK4 steps.
DESC coupling currently uses sampled geometry arrays or optional DESC path
loading; integer topology and remeshing remain static.  Therefore the DESC
examples should remain labeled reduced until the W7-X external parity,
production convergence, and guarded CPU-timing ledgers pass.

\subsection{Production criteria}

Before claiming a trusted linear stellarator run, the project needs:
\begin{enumerate}
  \item weighted stella-vs-solver \(k_y=0.3\) term-array parity, or a documented
  convention difference justified by arrays rather than scalar norms;
  \item a passing W7-X mode-structure gate against the matched stella fixture;
  \item a production-control W7-X convergence ladder in field-line length,
  \(N_z\), velocity resolution/backend, \(k_x/k_y\), timestep, and growth-window
  diagnostic;
  \item a guarded production CPU timing result after parity passes;
  \item an updated readiness ledger that allows production DESC optimization.
\end{enumerate}
