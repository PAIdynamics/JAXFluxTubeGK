...
- [ ] Run or capture the matched production-control GKW `cosin2` compact state trace and compare it against the solver `CycloneSourceTermTrace` before adding more diagnostic sign variants.
...
2026-06-02 update: the GKW-side state-history path is now prepared but not yet populated with a production fixture. `scripts/prepare_gkw_cosine2_run.py --state-trace` patches only the copied `diagnostic.F90` and writes `stellarator_gk_state_trace.dat`; `load_gkw_state_trace` and `compare_gkw_state_trace_to_source_term_trace` provide the Python ingestion/comparison contract.
...
- [ ] Run or capture the matched production-control GKW state trace and compare it against the solver source-term trace after Term VII and simple fused-`igh` sign paths are ruled out.
