submodule (timestep_m) timestep_init_gpu_s
    !! Contains implementation of the initialization of GPU-supported operators
    !! for timestep_t
...
#endif
    use op_coll_m, only: op_coll_t, op_coll_lbd_cpu_t, &
                         op_coll_fpl_cpu_t
    ...
    use op_bnd_cond_m, only: op_bnd_cond_mms_cpu_t, op_bnd_cond_neum_cpu_t
...
contains
    ...
    module subroutine initialize_operators_grid_gpu(this)
        ...
        allocate(op_mom_maxwells_eq_gpu_t :: this%op_mom_maxwells_eq)
        allocate(op_mom_ohms_law_gpu_t :: this%op_mom_ohms_law)
        ...
        select case(coll_type)
            ...
            case("bgk") ! BGK collision operator
                allocate(op_coll_bgk_gpu_t :: this%op_coll)
                allocate(op_mom_coll_gpu_t :: this%op_mom_coll)
            ...
            case("fpl") ! FPL collision operator
        ...
        if(this%run_mms) then
            ...
            select case(bnd_cond_type)
                case('dirichlet')
                    allocate(op_bnd_cond_dir_gpu_t :: this%op_bnd_cond)
                case('neumann')
                    allocate(op_bnd_cond_neum_gpu_t :: this%op_bnd_cond)
                case default
                    call handle_error(&
