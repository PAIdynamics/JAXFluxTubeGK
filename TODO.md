...
- use the new matched GKW `parallel_phi.dat` profile comparison to isolate the remaining parallel mode-structure gap, including GKW field-normalization, output-order, and parallel-boundary details not visible in compact `time.dat`;
- only patch GKW or add a restart/state-injection path for `cosine2` if the native `cosine` profile comparison cannot isolate the discrepancy;
