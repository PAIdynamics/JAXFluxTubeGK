submodule(op_solve_amps_law_m) op_solve_amps_law_s
    use mpi
...
contains
    ...
    module subroutine initialize(this, dcomm_handler, mesh)
        ...
        real(kind=GP), contiguous, pointer, dimension(:,:) :: lambda, xi, co
        ...
        !$omp shared(rho_ref, L_ref, co, xi, lambda, not_in_target, jacobian, &
        !$omp        RZ_indices)
        ...
        ! Only call update if GPU features of PARALLAX is used due to known bugs
        ! TODO: Remove this once the bugs are resolved in PARALLAX
        if(get_use_parallax_gpu_offload()) then
            ! Get the device pointers if applicable
    ...
    module subroutine apply(this, da_b_amps_law, da_A_par, residuum)
        ...
        real(kind=GP), contiguous, pointer, dimension(:,:) :: b_amps_law, A_par
        ...
        do k = lb_phi_stripped, ub_phi_stripped
            ...
            call this%solvers(k)%solve(rhs=b_amps_law(:size_RZ_plane, k), &
                                       sol=A_par(:size_RZ_plane, k), &
            ...
            if(iter < 0) then
                call handle_error("Ampere's law solver did not converge!", &
        ...
        if(this%dcomm_handler%is_master()) then
            write(logger_get_debug_channel(), *) &
                "    Ampere's law solver residuum: ", residuum
            if(iter > 0) then
                write(logger_get_debug_channel(), *) &
                    "    Ampere's law solver iterations: ", iter
            endif
        endif
    ...
    contains
        ...
        subroutine assign_array_pointers()
            ...
            if(get_use_parallax_gpu_data_explicit()) then
                b_amps_law => da_b_amps_law%get_readonly_device_pointer()
                A_par => da_A_par%get_device_pointer()
            else
                b_amps_law => da_b_amps_law%get_readonly_pointer()
                A_par => da_A_par%get_pointer()
            endif
        end subroutine
