...
\end{equation}
Using the GKW-native \(\texttt{finit='cosine'}\) initialization, the matched
profile comparison remains open:
...
This shows that the remaining Cyclone discrepancy includes a parallel
mode-structure mismatch, not only a scalar growth-window convention.  The
direct compact GKW comparison path uses \(\texttt{finit='cosine'}\); matching
the solver/Gyaradax \(\texttt{cosine2}\) initialization would require a local
GKW source patch or a restart/state-injection path because the original GKW
\texttt{finit} selector has no native \(\texttt{cosine2}\) branch.
