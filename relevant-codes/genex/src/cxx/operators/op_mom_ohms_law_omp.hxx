#ifndef OP_MOM_OHMS_LAW_OMP_HXX
#define OP_MOM_OHMS_LAW_OMP_HXX
...
// with OpenMP on CPU
class op_mom_ohms_law_omp_t: public op_mom_ohms_law_gpu_t
{
public:
    // Constructor of the OpenMP child class
    op_mom_ohms_law_omp_t(const mesh_5d_t& mesh)
    : op_mom_ohms_law_gpu_t{mesh} {};

    // Destructor of the OpenMP child class
    ~op_mom_ohms_law_omp_t() override {};

    // Copy constructor is disabled
    op_mom_ohms_law_omp_t(const op_mom_ohms_law_omp_t&) = delete;

    // Copy-assignment operator is disabled
    op_mom_ohms_law_omp_t& operator=(const op_mom_ohms_law_omp_t&) = delete;
        ...
        data_array_t<real_t, 2>& b_ohms_law) const override
    {
        ...
        data_array_omp_t<real_t, 3> send_buffer(
            {lb_stripped[0], lb_stripped[1], 1},
            {ub_stripped[0], ub_stripped[1], 2}, 0.0);
        data_array_omp_t<real_t, 3> receive_buffer(
            {lb_stripped[0], lb_stripped[1], 1},
