#ifndef MAILBOX_OMPX_HXX
#define MAILBOX_OMPX_HXX

#include "genex_cxx_env.hxx"
#include "mailbox.hxx"
#include <omp.h>

// C++ class which corresponds to the Fortran class mailbox_t
// with OpenMP offload
...
class mailbox_ompx_t: public mailbox_t
{
private:
    ...
    inline void update_device_inbox(int32_t partner_index) override
    {
        ...
        #pragma omp target update \
            to(this->inboxes_ptr[start_idx:this->n_mailbox_cells])
#endif
    }

public:
    // Parameterized constructor of the OpenMP offload child class
    mailbox_ompx_t(const int32_t n_mailbox_cells,
    ...
    ~mailbox_ompx_t() override
    {
        if (this->is_allocated_on_device) this->deallocate_device();
    }

    // Copy constructor is disabled
    mailbox_ompx_t(const mailbox_t&) = delete;

    // Copy-assignment operator is disabled
    mailbox_ompx_t& operator=(const mailbox_t&) = delete;
    ...
    void allocate_device() override
    {
        ...
        #pragma omp target enter data map(alloc: this->outboxes_ptr[:mb_size], \
    ...
    void deallocate_device() override
    {
        ...
        #pragma omp target exit data map(delete: this->inboxes_ptr[:mb_size], \
                                                 this->outboxes_ptr[:mb_size])
        this->dmem_debug(dmd::mode_t::DEALLOC);
    }

    // Pack the elements in the part of 2D data_array_t
                      ...
                      const int32_t partner_index) override
    {
        ...
        #pragma omp target teams distribute parallel for simd collapse(2) \
            ...
            map(to: lb[:2], ub[:2]) shared(lb, ub, outbox, data)
    ...
    // Pack the elements in the part of 4D data_array_t
    // via OpenMP offload on GPU
                      ...
                      const int32_t partner_index) override
    {
        ...
        #pragma omp target teams distribute parallel for simd collapse(4) \
            default(none) defaultmap(none) \
            map(to: lb[:4], ub[:4], strides[:4]) \
            shared(lb, ub, strides, outbox, data)
    ...
    // Pack the elements in the part of 5D data_array_t
    // via OpenMP offload on GPU
    ...
    // Unpack the elements in the part of 2D data_array_t
    // via OpenMP offload on GPU
                        ...
                        const int32_t partner_index) override
    {
        ...
        #pragma omp target teams distribute parallel for simd collapse(2) \
            default(none) defaultmap(none) \
            map(to: lb[:2], ub[:2]) shared(lb, ub, inbox, data)
        for (int32_t k = lb[1]; k <= ub[1]; k++)
        for (int32_t i = lb[0]; i <= ub[0]; i++)
        {
            int32_t ctr = i - lb[0] + (k - lb[1]) * (ub[0] - lb[0] + 1);
    ...
    // Unpack the elements in the part of 4D data_array_t
    // via OpenMP offload on GPU
                        ...
                        const int32_t partner_index) override
    {
        ...
        #pragma omp target teams distribute parallel for simd collapse(4) \
            default(none) defaultmap(none) \
             map(to: lb[:4], ub[:4], strides[:4]) \
            shared(lb, ub, strides, inbox, data)
        for (int32_t o = lb[3]; o <= ub[3]; o++)
        for (int32_t m = lb[2]; m <= ub[2]; m++)
        for (int32_t k = lb[1]; k <= ub[1]; k++)
        for (int32_t i = lb[0]; i <= ub[0]; i++)
    ...
    // Unpack the elements in the part of 5D data_array_t
    // via OpenMP offload on GPU
