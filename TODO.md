...
- [ ] Use the compact state-trace result to decide whether the next discriminator must be a full selected-mode state dump or a term-resolved GKW RHS/source trace; compact state/field norms no longer explain the CBC growth/profile gap by themselves.
...
- [ ] Continue from the compact state-trace result toward a full selected-mode state dump or GKW term-resolved RHS/source trace; norm-level state/field parity is not enough to close the profile/growth discrepancy.
