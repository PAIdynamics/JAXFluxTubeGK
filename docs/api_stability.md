...
`optimal-fusion` is pre-1.0 software, but the `0.1.x` series maintains a small
...
- Public immutable specifications in `stellarator_gk.types`, including grid,
  species, topology, geometry-scalar, and solver-control records.
- Grid builders in `stellarator_gk.grids` and the provider-neutral geometry
  contracts in `stellarator_gk.geometry`.
- Linear residual precompute/build/evaluation entry points in
  `stellarator_gk.solver`, fixed-step/adaptive integration results in
  `stellarator_gk.time_advance`, and public diagnostics.
- `BenchmarkTarget` and its residual/cost helpers in `stellarator_gk.targets`.
...
The canonical imports for this surface are from `stellarator_gk` or the
focused subsystem module. Symbols present in `stellarator_gk.__all__` define
the compact top-level API for this series.

## Validation surface

External-reference workflows live under `stellarator_gk.validation`:
...
benchmark names remain available lazily from `stellarator_gk` for `0.1.x`
compatibility; they are deliberately absent from `stellarator_gk.__all__` and
new code should use the validation namespace.

## Experimental/internal surface

Underscore-prefixed helpers, the legacy `stellarator_gk.benchmarks` module,
