...
loading; integer topology and remeshing remain static.  W7-X external parity
and guarded timing now pass for the converged linear branch, but the DESC
examples remain reduced until Priority 4 validates gradients through a real MHD
solve and defines remeshing/topology-change behavior.
...
\end{enumerate}
Production design optimization additionally requires the Priority 4 real-MHD
gradient and topology gates.  A broader low-\(k_y\) spectral claim additionally
requires converged independent reference windows.
