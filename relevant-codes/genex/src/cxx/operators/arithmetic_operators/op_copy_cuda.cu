...
__global__ void kernel_copy_real_core_cuda(const int64_t size_y,
...
__global__ void kernel_copy_integer_core_cuda(const int64_t size_y,
                                              const int* __restrict__ x_device,
...
int32_t op_copy_cuda_t::apply(const int64_t size_y,
                                    ...
                                    real_t* __restrict__ y) const
{
    ...
    kernel_copy_real_core_cuda<<<num_blocks, block_size>>>
        (size_y, x_device, y_device);
                                    ...
                                    int32_t* __restrict__ y) const
{
    ...
    cudaMemcpy(x_device, x, size, cudaMemcpyHostToDevice);
    kernel_copy_integer_core_cuda<<<num_blocks, block_size>>>
        (size_y, x_device, y_device);
    cudaMemcpy(y, y_device, size, cudaMemcpyDeviceToHost);
