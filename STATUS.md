...
real frequency, and phase-aligned complex \(\phi(z)\). The W7-X production
readiness ledger is now generated by
`scripts/run_w7x_production_readiness_gate.py`; the committed result passes the
reduced convergence regression and remains `blocked_external_reference` until
the matched external W7-X fixture and true production CPU timing exist. The
