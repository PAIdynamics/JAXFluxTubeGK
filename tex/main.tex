...
\title{JAXFluxTubeGK: Physical Model and Numerical Scheme for a Differentiable Flux-Tube Stellarator Gyrokinetic Solver}
\author{Mohsen Sadr}
\date{\today}
...
When conventions are ambiguous, use the following order of authority.  GKW
and Gyaradax are external reference codes: this project follows their
published physics and, where useful, their source conventions, but does not
vendor their source in this repository.
\begin{enumerate}
  \item The published GKW paper~\cite{peeters2009gkw}.
  \item GKW's own Fortran source and sample cases, obtained as an external,
  revision-pinned checkout (see \path{docs/dependencies.md}).
  \item The Gyaradax JAX implementation~\cite{gyaradax}, likewise an
  external, revision-pinned checkout.
...
The filenames below are GKW's own external Fortran modules, referenced here
purely to record which module's physics and conventions each part of this
project follows.
...
The same internal geometry can be exported to the \texttt{eik}-style
contract used for stellarator parity checks:
\begin{equation}
  \mathcal{E}_{\mathrm{eik}}
  =
  \{
    \theta,\ B,\ \grad_\parallel,\ \mathtt{gds2},\ \mathtt{gds21},\ \mathtt{gds22},
    \mathtt{gbdrift},\ \mathtt{gbdrift0},\ \mathtt{cvdrift},\ \mathtt{cvdrift0}
  \}.
...
\[
  ...
  (\mathtt{gds2},\mathtt{gds21},\mathtt{gds22})=(g^{yy},g^{xy},g^{xx}),
\]
and preserves the summed drift coefficients through
\[
  \mathtt{gbdrift}+\mathtt{cvdrift}=D_y,\qquad
  \mathtt{gbdrift0}+\mathtt{cvdrift0}=D_x.
\]
...
external \texttt{eik} fixtures without claiming a curvature split that was not
provided.
Some external \texttt{eik}/GIST text fixtures use an alternate drift-column
ordering; when reading that variant, the columns are read in the order
\[
  \theta,\ B,\ \grad_\parallel,\ \mathtt{gds2},\ \mathtt{gds21},\ \mathtt{gds22},
  \mathtt{cvdrift},\ \mathtt{cvdrift0},\ \mathtt{gbdrift},\ \mathtt{gbdrift0},
\]
...
\begin{equation}
  ...
  \label{eq:linear-rhs}
\end{equation}
Term numbers follow the GKW/Gyaradax convention.  Term III, the nonlinear ExB term, is omitted in the linear baseline and documented in Appendix~\ref{sec:nonlinear-extension}.
...
\end{equation}
The gyrophase factor is absorbed into the chosen normalization; if a benchmark requires an explicit \(2\pi\), it should enter the documented velocity measure, not an individual physics term.  Velocity collocation is the selected production CPU representation: Chebyshev is the general reduced/differentiable path, GKW finite differences retain convention parity, and the midpoint/Gauss--Laguerre recipe is used by the source-matched W7-X gate.  Hermite--Laguerre remains an experimental reduced discriminator until it is integrated with the complete stellarator residual and field solve and passes comparable convergence, parity, gradient, and timing gates.
...
above.  The core package in \path{src/jax_fluxtube_gk/} contains PyTree parameter
...
Appendix~\ref{sec:nonlinear-extension}.
...
\begin{itemize}
  ...
  \item eik geometry checks pass for DESC fixtures, solver export, eik
  import, and independent VMEC/GIST sources;
...
nonnegative \(k_\perp^2\), eik-export compatibility, and an optional
...
\path{fixtures/w7x_itg_reduced_benchmark/}.  It uses a W7-X eik geometry and
adiabatic-electron ITG input as provenance.  Its scalar
...
\subsection{Nonlinear turbulence acceptance status}
\label{sec:nonlinear-acceptance-status}

The electrostatic nonlinear-turbulence acceptance campaign
(Appendix~\ref{sec:nonlinear-extension}) is implemented but not scientifically
closed; this is a compute-evidence gap, not a missing operator.  Three of its
four gates have passing rungs:
\begin{itemize}
  \item a stationary phase-space resolution ladder, \(16{\times}16{\times}8
  \to 20{\times}20{\times}10\) at fixed Fourier/physics contract, changes the
  mean local heat-flux proxy by \(3.88\%\), below the \(15\%\) gate;
  \item a three-lineage ensemble with distinct initialization roots (seed,
  amplitude, zonal fraction) is stationary with maximum relative deviation
  \(2.04\%\) from the ensemble mean, below the \(15\%\) gate;
  \item the bandwidth-preserving domain-convergence ladder is \emph{not}
  converged: the stationary \(65{\times}33\to129{\times}65\),
  \(k_{y,\min}=0.0125\to0.00625\) finest-pair change is \(25.65\%\), above the
  \(15\%\) gate, so a further \(257{\times}129\), \(k_{y,\min}=0.003125\)
  bandwidth-preserving rung is required;
  \item independent nonlinear parity against a revision-pinned external
  reference gyrokinetic code remains deferred: it needs a CUDA-capable host,
  which was not available for this campaign.
\end{itemize}
The required next domain rung was attempted on a CPU-only development
workstation and ran for approximately \(19.5\) hours without a solver or
memory error, but did not reach its checkpoint/report phase before being
deliberately terminated; no partial artifact was produced or retained.  Both
remaining gates need external compute this project does not currently have
access to (a CPU cluster or GPU/CUDA host), not further algorithmic
development.  Unrestricted equilibrium-shape optimization
(Appendix~\ref{sec:nonlinear-extension}) is explicitly gated behind all four
nonlinear-acceptance items and is not attempted before they pass.
...
This section reports two distinct classes of executed optimization evidence.
The first is a genuine multi-step, end-to-end differentiable optimization on
analytic circular geometry: every iteration differentiates through the full
residual, field solve, and time advance via \texttt{jax.grad}.  The second is
a real MHD-coupled boundary-harmonic scan through the installed DESC, GVEC,
and VMEC++ providers: a fresh equilibrium is re-solved at each of three
boundary-scale points and the resulting geometry is fed through the unchanged
gyrokinetic objective.  The two are not the same claim.  The first exercises
the differentiable optimizer on a fixed, cheap analytic geometry; the second
exercises the live equilibrium/geometry/objective pipeline but only as a
three-point central-difference sensitivity, not a multi-step optimizer, and
is not a claim of converged shape optimization.  Section~\ref{sec:current-status}
already documents that end-to-end automatic differentiation \emph{through} a
live equilibrium solve is a separate, unimplemented milestone; both result
classes below are consistent with that boundary.

\subsubsection{Differentiable knob optimization}

\texttt{examples/generate\_optimization\_figures.py} runs three
\(1000\)-iteration gradient-descent cases (\(A\), \(B\), \(C\)) that minimize
\(\tfrac12(\gamma_{\text{obj}}-\gamma_{\text{target}})^2\) with
\(\gamma_{\text{target}}=0\) (drive to marginal stability) by differentiating
through \(q\), \(\hat s\), \(R/L_n\), \(R/L_T\), and the other circular-geometry
knobs in \texttt{OptimizationKnobs}.  Case \(A\) targets a single selected
\(k_y\) branch on a two-mode grid; case \(B\) targets a single selected branch
on a three-mode grid with a different starting equilibrium; case \(C\) targets
the differentiable soft/hard maximum over the nonzonal branches
(\texttt{scalar\_mode="max\_nonzonal"}) rather than one fixed mode index.  All
three converge:
...
\begin{tabular}{lrrrr}
\toprule
Case & Initial \(|\text{obj.\ error}|\) & Final \(|\text{obj.\ error}|\) & \(q\): initial\(\to\)final & \(\hat s\): initial\(\to\)final \\
\midrule
A & \(6.47\times10^{-3}\) & \(3.78\times10^{-8}\) & \(1.250\to1.261\) & \(0.450\to0.448\) \\
B & \(7.85\times10^{-2}\) & \(<10^{-12}\) & \(1.350\to1.371\) & \(0.650\to0.661\) \\
C & \(6.72\times10^{-2}\) & \(<10^{-12}\) & \(1.450\to1.468\) & \(0.750\to0.759\) \\
\bottomrule
\end{tabular}
\caption{Reduced circular-geometry gradient-descent cases: absolute objective
error and geometry-knob drift from \(1000\) \texttt{jax.value\_and\_grad}
steps at learning rate \(10^{-3}\).  Case A's selected-\(k_y\) growth rate
moves from \(1.138\times10^{-1}\) to \(2.748\times10^{-4}\); case C's
differentiable nonzonal maximum reaches its zero target while the specific
selected branch (index 1, not the arg-max) settles at a distinct, more stable
value \(-3.166\times10^{-1}\), showing the soft-maximum objective correctly
tracks the least-stable branch rather than a fixed mode index.}
\label{tab:optimization-cases}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{optimization_objectives.pdf}
\caption{Absolute objective error versus iteration for cases A--C (log
scale).  All three cases converge monotonically to within numerical noise of
the marginal-stability target.}
\label{fig:optimization-objectives}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{optimization_growth_rates.pdf}
\caption{Selected-mode and nonzonal-maximum growth-rate change relative to
each case's initial value.  Case C shows the distinct selected-branch and
max-nonzonal-branch trajectories referenced in
Table~\ref{tab:optimization-cases}.}
\label{fig:optimization-growth-rates}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{optimization_geometry_knobs.pdf}
\caption{Safety-factor \(q\) and magnetic-shear \(\hat s\) drift from their
initial values under the gradient-descent update.  Knob changes stay small
(a few percent) over the full trajectory, consistent with driving toward a
nearby marginal-stability point rather than an unconstrained large excursion.}
\label{fig:optimization-geometry-knobs}
\end{figure}

\subsubsection{Live-provider design-loop results}

Each of the three geometry-optimization notebooks
(\path{examples/desc_geometry_optimization.ipynb},
\path{examples/gvec_geometry_optimization.ipynb},
\path{examples/vmecpp_geometry_optimization.ipynb}) was executed end to end
against its installed live provider: DESC's named \texttt{DSHAPE}
configuration, VMEC++'s named \texttt{w7x-standard} configuration, and GVEC's
tracked VMEC-initialized W7-X parameterization.  Each notebook scales one
boundary Fourier harmonic by \(\{1-\delta,1,1+\delta\}\), re-solves the
equilibrium at every scale, reconstructs the flux-tube geometry, and
re-evaluates the unchanged reduced gyrokinetic objective, verifying fixed
optimization topology between solves with
\texttt{assert\_fixed\_optimization\_topology}.
...
\toprule
Provider & Harmonic & Scale \(\delta\) & Growth rate & Frequency & Central-diff.\ sensitivity \\
\midrule
DESC & \(R_{2,0}\) & \(0.01\) &
  \(0.15295\to0.15287\) & \(-0.0928\to-0.0907\) & \(-4.14\times10^{-3}\) \\
GVEC & \(X1^{\cos}_{1,1}\) & \(0.005\) &
  \(0.27422\to0.27414\) & \(-0.0498\to-0.0498\) & \(-7.63\times10^{-3}\) \\
VMEC++ & \((m,n)=(1,0)\) & \(0.005\) &
  \(-0.21841\to-0.22166\) & \(0.1155\to0.1160\) & \(-3.26\times10^{-1}\) \\
\bottomrule
\end{tabular}%
}
\caption{Live-provider boundary-harmonic scans.  ``Growth rate''/``Frequency''
span the three evaluated scale points \(1-\delta\) to \(1+\delta\); the
central-difference sensitivity is \((\text{obj.}(1+\delta)-\text{obj.}(1-\delta))/(2\delta)\)
of the scalar reduced-growth objective.  GVEC's minimizer was run at a bounded
\texttt{MaxIter=100} and did not reach its force-tolerance criterion
(\(|\text{force}|\approx2.3\times10^{-3}\) against a \(10^{-8}\) target) at any
of the three scales, so its geometry is a diagnostic near-equilibrium, not a
converged one -- consistent with each notebook's own framing of this scan as
an interface/sensitivity demonstration rather than an accepted design point.}
\label{tab:live-provider-scans}
\end{table}

None of the three scans differentiates through the equilibrium solve itself;
each is an outer-loop, finite-difference sensitivity through independently
re-solved equilibria, matching the ``Outer loop (available now)'' path
described in the notebooks rather than the deferred end-to-end automatic-
differentiation path.

\paragraph{Extended outer-loop hill-climb.}
Beyond the single 3-point sensitivity in Table~\ref{tab:live-provider-scans},
each notebook was extended into a longer outer-loop hill-climb targeting 100
iterations: the same central-difference sensitivity steps the boundary scale
every iteration, and the loop fails closed -- stopping early but keeping
every completed iteration -- on a non-finite objective, a raised solver
exception, or a wall-clock budget.  DESC completed 46 iterations before a
boundary-harmonic scale of \(1.0037\) produced a non-finite objective from a
failed equilibrium solve; GVEC completed 48 iterations before its
900-second budget; VMEC++, whose equilibrium solve is roughly two orders of
magnitude more expensive per evaluation than DESC or GVEC at this
resolution, completed 15 iterations within an extended 1800-second
budget using a finer 3-iteration frame stride (stopping after 1877s against
that budget).  None of these stops is a bug: each is the fail-closed loop
behaving exactly as designed when a real equilibrium solver becomes
unreliable or a shared compute budget is exhausted, and each is reported
rather than hidden.

Figures~\ref{fig:desc-boundary-evolution}--\ref{fig:vmecpp-boundary-evolution}
show the real solved 3D boundary surface at the recorded frame stride,
reconstructed directly from each provider's own boundary representation
(DESC's \texttt{FourierRZToroidalSurface} via \texttt{surface.compute}, GVEC's
Cartesian \texttt{state.evaluate("pos", ...)} at \(\rho=1\), and VMEC++'s
\texttt{rbc}/\texttt{zbs} Fourier reconstruction) rather than a schematic.
Figures~\ref{fig:desc-loss-trace}--\ref{fig:vmecpp-loss-trace} show the
corresponding objective trajectories: DESC and GVEC both decrease
monotonically toward the marginal-stability target as the boundary scale
increases; VMEC++'s objective also decreases monotonically, from
\(-0.22007\) to \(-0.23109\), as its boundary scale moves from \(1.0\) to
\(1.0394\) over the same 15 iterations -- a visibly larger single-iteration
step than DESC or GVEC, consistent with its order-of-magnitude larger
central-difference sensitivity in Table~\ref{tab:live-provider-scans}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{desc_boundary_evolution.png}
\caption{DESC \(R_{2,0}\) boundary evolution across 46 completed outer-loop
iterations, shown every 25 iterations.  Each surface is evaluated directly
from the solved equilibrium's boundary via \texttt{surface.compute}.}
\label{fig:desc-boundary-evolution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{desc_initial_live_design_scan.png}
\caption{DESC reduced GK objective and \(R_{2,0}\) coefficient scale across
the same 46-iteration trajectory.}
\label{fig:desc-loss-trace}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{gvec_boundary_evolution.png}
\caption{GVEC \(X1^{\cos}_{1,1}\) boundary evolution across 48 completed
outer-loop iterations, shown every 25 iterations.  Each surface is the real
Cartesian position of the solved GVEC state at \(\rho=1\).}
\label{fig:gvec-boundary-evolution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{gvec_initial_live_design_scan.png}
\caption{GVEC reduced GK objective and maximum force residual across the
same 48-iteration trajectory.  The force residual stays flat and bounded
away from GVEC's own convergence tolerance throughout, consistent with the
bounded-\texttt{MaxIter} diagnostic framing used everywhere else in this
section.}
\label{fig:gvec-loss-trace}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{vmecpp_boundary_evolution.png}
\caption{VMEC++ paired \((m,n)=(1,0)\) boundary evolution across 15
completed outer-loop iterations, shown every 3 iterations because of
VMEC++'s substantially higher per-solve cost at this resolution.  Each
surface is reconstructed directly from the scaled \texttt{rbc}/\texttt{zbs}
Fourier coefficients VMEC++ solved for.}
\label{fig:vmecpp-boundary-evolution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{vmecpp_initial_live_design_scan.png}
\caption{VMEC++ reduced GK objective and paired \((m,n)=(1,0)\) scale across
the same 15-iteration trajectory.}
\label{fig:vmecpp-loss-trace}
\end{figure}
...
\begin{enumerate}
  ...
  \item DESC/stella eik and geometry contracts;
...
accepts these tensors without depending on stella.  Native stella traces
verify every stage of this construction: the driver $D$, the response $R$
(built independently from the physical velocity and magnetic grids, species
mass ratios, pair collision frequencies, gyroaverages, and $\Delta_j$), and
their low-rank contraction reproduce tens of thousands of native reference
coefficients and the native solved-$\Psi$ action at relative $L_2$ errors
between $10^{-11}$ and $10^{-16}$, independent of the equilibrium state.

The remaining piece, the differential test-particle matrix $C_{\mathrm{tp}}$,
is independently constructed rather than read from stella: the
Gyaradax-matched nine-point stencil; the mu- and parallel-velocity
finite-difference blocks, including their interior and boundary closures on
uniform and nonuniform grids; the mode-dependent gyro diagonal; and the
underlying analytic collision-frequency layer (speed, Maxwellian, parallel
diffusion, deflection, mixed diffusion).  Each piece matches the corresponding
native stella matrices and, for the four ordered species channels, the
all-channel matrix and final 2,808-value phase-space state, at relative $L_2$
errors between $10^{-14}$ and $10^{-17}$.  A differentiable Woodbury solve
combines $I-\Delta t\,C_{\mathrm{tp}}$ with $D$ and $R$ in stella's implicit
ordering and agrees with a directly materialized dense backward-Euler system
at roundoff.

The resulting production implicit collision backend -- selectable as a split
step alongside the linear residual -- is therefore validated end to end
against native stella at near-roundoff precision, closing the collision
implementation item.  Together with the independent electromagnetic parity
gates in Sec.~\ref{sec:current-status}, production collisional and linear
electromagnetic physics are implemented and validated for the accepted scope;
nonlinear-turbulence and unrestricted design-optimization evidence remain the
open items tracked in Sec.~\ref{sec:nonlinear-acceptance-status}.
...
\newblock arXiv preprint arXiv:2604.06085, 2026.
