module op_solve_ohms_law_m
    ...
    type, public, extends(op_base_t) :: op_solve_ohms_law_t
        !! Operator to solve Ohm's law on the CPU and GPU
    ...
    interface
        ...
        module subroutine apply(this, da_lambda_ohms_law, da_b_ohms_law, &
                                da_E_par, residuum)
            !! Solves Ohm's law for the given right hand side b and Helmholtz
            !! coefficient lambda with initial guess E_par.
            !! Solution will be put into E_par.
            class(op_solve_ohms_law_t), target, intent(inout) :: this
            !! Instance of the type
            class(data_array_2d_t), intent(in) :: da_lambda_ohms_law
            !! lambda array of the Helmholtz problem
