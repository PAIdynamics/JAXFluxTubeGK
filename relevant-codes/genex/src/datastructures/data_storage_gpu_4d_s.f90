submodule (data_storage_m) data_storage_gpu_4d_s
    ...
    interface
        integer(kind=C_INT32_T) function cbind_data_storage_4d_initialize( &
            dcomm_handler_cxx_pptr, ds_data, da_cxx_pptr, ds_cxx_pptr) &
            bind(C, name="cbind_data_storage_4d_initialize")
            !! Fortran/C++ interoperable routine for the initialization
            !! of data_storage_gpu_4d_t C++ class
        ...
        integer(kind=C_INT32_T) function cbind_data_storage_4d_start_exchange( &
            ds_cxx_pptr) bind(C, name="cbind_data_storage_4d_start_exchange")
        ...
        integer(kind=C_INT32_T) function cbind_data_storage_4d_finish_exchange(&
            ds_cxx_pptr) bind(C, name="cbind_data_storage_4d_finish_exchange")
...
contains
    ...
    module subroutine initialize_gpu_4d(this, dcomm_handler, n_mom, n_spec, &
                                        init_value)
        ...
        ! Initialize data_storage_gpu_4d_t C++ class,
        ! including deep copy to the device
        ierr = cbind_data_storage_4d_initialize( &
    ...
    module subroutine start_exchange_gpu_4d(this)
        ...
        ierr = cbind_data_storage_4d_start_exchange(this%data_storage_cxx_pptr)

        if(ierr /= 0) then
            call handle_error_gpu(GPU_ERR_START_EXCH, __LINE__, __FILE__)
        endif

        call profiler_inject("pack_4d", ierr)
        call profiler_inject("send_4d", ierr)
    ...
    module subroutine finish_exchange_gpu_4d(this)
        ...
        ierr = cbind_data_storage_4d_finish_exchange(this%data_storage_cxx_pptr)

        if(ierr /= 0) then
            call handle_error_gpu(GPU_ERR_FINISH_EXCH, __LINE__, __FILE__)
        endif

        call profiler_inject("receive_4d", ierr)
        call profiler_inject("unpack_4d", ierr)
    ...
    module subroutine finalize_gpu_4d(this)
        ...
        ierr = cbind_data_storage_4d_finalize(this%data_storage_cxx_pptr)
