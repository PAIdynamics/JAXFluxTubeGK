#ifndef OP_BND_COND_NEUM_OMPX_HXX
#define OP_BND_COND_NEUM_OMPX_HXX
...
// op_bnd_cond_neum_gpu_t with OpenMP offload on GPU
class op_bnd_cond_neum_ompx_t: public op_bnd_cond_neum_gpu_t
{
public:
    // Constructor of the OpenMP offload child class
    op_bnd_cond_neum_ompx_t(const dcomm_handler_t& dcomm_handler,
    ...
    ~op_bnd_cond_neum_ompx_t() override
    {
        int32_t len1 = this->max_num_compute_buf_core * this->size_phi_s;
        int32_t len2 = this->max_num_ghost_core * this->size_phi_s;
        ...
        #pragma omp target exit data \
            map(delete: this->number_bnd_points_ptr[:this->size_phi_s], \
                        this->ind_ghost_core_ptr[:len2], \
                        this->ind_compute_buf_core_ptr[:len1], \
                        this->max_num_ghost_core, \
                        this->max_num_compute_buf_core, \
    ...
    op_bnd_cond_neum_ompx_t(const op_bnd_cond_neum_ompx_t&) = delete;

    // Copy-assignment operator is disabled
    op_bnd_cond_neum_ompx_t& operator=(const op_bnd_cond_neum_ompx_t&) = delete;
                  ...
                  data_array_t<real_t, 2>& b_ohms_law) override
    {
        ...
        data_array_ompx_t<real_t, 4> buf_bnd(
        ...
        #pragma omp target teams distribute parallel for simd collapse(2) \
            default(none) defaultmap(none) \
            shared(lb, ub, lb_stripped, ub_stripped, \
                   mesh, b_qn_eq, b_amps_law, b_ohms_law)
        for (int32_t k = lb_stripped[1]; k <= ub_stripped[1]; k++)
