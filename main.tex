...
nonzero zonal \(k_x\rho_s=0.025\) mode, and can apply benchmark-controlled
spectral modal damping, with the default parallel damping motivated by
GKW/Gyaradax \(\texttt{disp\_par}=0.01\).  Passing this gate requires both the
reference residual and a small late-window spread.  In the present code the
gate is executable but still open; strong ad hoc velocity filtering can suppress
recurrence but damps the residual itself, so it is not accepted as production
validation.
...
\(k_x\rho_s=0.025\) mode, and the default parallel modal damping
\(\nu_z=0.01\).  Over \(t_{\mathrm{end}}\in[0.02,0.10]\), the measured
...
The right panel summarizes five executable gates.  The imported GX/GS2 eik
metric contract and the DESC DSHAPE geometry-to-eik export contract both pass
with zero residual.  The reduced RH endpoint, true RH plateau, and reduced
Cyclone growth gates remain open with normalized residuals
\(9.28\times 10^2\), \(9.28\times 10^2\), and \(1.88\times 10^3\),
respectively.  This figure is the current validation ledger: stellarator
geometry can now enter through the DESC array path and leave through a GX/eik
metric/drift contract, while the remaining open work is the RH/Cyclone
physics/numerics parity problem.
...
\begin{figure}[htbp]
  ...
  values below one pass the tolerance.  The GX/GS2 imported eik metric and
  DESC geometry eik-export contracts pass; RH and Cyclone remain deliberately
  open against production tolerances.}
