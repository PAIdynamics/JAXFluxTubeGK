...
frequency curves from \texttt{omega\_kxkyt}, and GX/GS2 eik-style geometry
tables.  These references are used as objective targets and parser tests now;
full solver-to-reference RH, Cyclone, and eik parity runs remain the next
validation gate.
...
\[
  ...
  \right).
\]
With the present reduced executable gates, the GX/GS2 eik metric import passes
the solver \(\kperp^2\) contract exactly, while the RH and Cyclone gates are
marked open against production tolerances.  This is intentional: it prevents
DESC-driven optimization studies from being mistaken for validated physical
predictions before the RH long-time residual and CBC selected-\(k_y\) growth
rate are brought onto the reference ladder.
The current implementation now also includes three validation-hardening tools.
...
\(\texttt{disp\_par}=1\).  A production-control runner stores only
per-window amplitudes and fits the late-time logarithmic amplitude, so the
documented \(N_z=144\), \(N_{v_\parallel}=64\), \(N_\mu=16\) setup can be run
without retaining a full field history.
...
The right panel summarizes six executable gates.  The imported GX/GS2 eik
metric contract, the DESC DSHAPE geometry-to-eik export contract, and a
three-fixture GX/VMEC GIST eik suite pass with zero residual.  The true RH
late-plateau gate now also passes with normalized residual \(-6.87\times10^{-1}\).
The reduced RH endpoint and reduced Cyclone growth gates remain open with
normalized residuals \(9.25\times10^2\) and \(2.15\times10^2\), respectively.
This figure is the current validation ledger: stellarator geometry can now
enter through the DESC array path and be audited through GX/GIST/eik metric and
drift contracts, while the remaining open production-physics item is the
Cyclone selected-\(k_y\) growth-rate parity problem.
...
\begin{figure}[htbp]
  ...
  geometry eik-export, and GX/VMEC GIST external eik-suite contracts pass; RH
  and Cyclone remain deliberately open against production tolerances.}
