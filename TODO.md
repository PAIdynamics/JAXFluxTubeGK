...
- [ ] Use the `igh` series result to target the remaining cumulative state-history gap: compare the actual solver/GKW state or residual trace under the promoted `gkw_igh` backend, ideally with a matched full-state/restart dump or a compact source-term time trace rather than more diagnostic sign variants.
...
- [ ] Use a matched state-history, source-term time trace, or GKW restart/final-state dump to isolate the remaining cumulative solver/GKW history gap after Term VII and simple fused-`igh` sign paths are ruled out.
