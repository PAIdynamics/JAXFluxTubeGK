#ifndef HELPERS_DATA_ARRAY_3D_CXX
#define HELPERS_DATA_ARRAY_3D_CXX
...
// Simple 3D addition operation with data_array to test the array getter and
// setter via OpenMP on CPU
int32_t data_array_3d_add_cxx(real_t add_const, data_array_t<real_t, 3>& da)
{
...
int32_t data_array_3d_add_acc(real_t add_const, data_array_t<real_t, 3>& da)
{
    #pragma acc parallel default(none) present(add_const, da) copyin(add_const)
    {
        #pragma acc loop independent collapse(3) firstprivate(add_const)
        for (int32_t l = da.get_lbound(3); l <= da.get_ubound(3); l++)
        for (int32_t k = da.get_lbound(2); k <= da.get_ubound(2); k++)
        for (int32_t i = da.get_lbound(1); i <= da.get_ubound(1); i++)
        {
...
int32_t data_array_3d_add_ompx(real_t add_const,
                               data_array_t<real_t, 3>& da)
{
...
// Interoperable routine to add the 3d array in data array with a given constant
int32_t cbind_data_array_3d_add(real_t add_const,
                                data_array_t<real_t, 3>** da_cxx_pptr)
{
    // Assign the C++ class instances
    data_array_t<real_t, 3>& da = *(*da_cxx_pptr);

    // Return 0 for success and 1 for error
    switch (params_gpu_offload::get_backend())
    {
        case params_gpu_offload::backend_t::CPU:
            return data_array_3d_add_cxx(add_const, da);
#ifdef ENABLE_OPENACC
        case params_gpu_offload::backend_t::ACC:
            return data_array_3d_add_acc(add_const, da);
#endif
#ifdef ENABLE_OPENMPX
        case params_gpu_offload::backend_t::OMPX:
            return data_array_3d_add_ompx(add_const, da);
#endif
        default:
            return 1;
    }
