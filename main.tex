...
\end{equation}
The gate uses the normalized GKW cell-centered \(s\)-grid, tracks the
nonzero zonal \(k_x\rho_s=0.025\) mode, and applies the in-residual
\(\texttt{disp\_par}=0.01\) recurrence-control term in
\eqref{eq:gkw-disp-par}.  Passing this gate requires both the reference
residual and a small late-window spread.  In the present code the gate is
executable but still open; post-step modal damping is disabled by default
because it can suppress recurrence while also damping the residual itself.
...
\(\texttt{disp\_par}=0.01\).  Over \(t_{\mathrm{end}}\in[0.02,0.10]\), the measured
metric decreases only from \(0.9999\) to \(0.9988\), far from the documented
...
three-fixture GX/VMEC GIST eik suite pass with zero residual.  The reduced RH
endpoint, true RH plateau, and reduced Cyclone growth gates remain open with
normalized residuals \(9.28\times 10^2\), \(9.28\times 10^2\), and
\(1.88\times 10^3\), respectively.  This figure is the current validation
ledger: stellarator geometry can now enter through the DESC array path and be
audited through GX/GIST/eik metric and drift contracts, while the remaining
open work is the RH/Cyclone physics/numerics parity problem.
