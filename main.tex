...
and true RH late-plateau metric pass their stated tolerances.  The short RH
endpoint smoke test and Cyclone selected-\(k_y\) gate remain marked open.  A
new CBC term audit passes for the drift, drive, field-drive, boundary-map, and
grid-normalization conventions.  This is intentional bookkeeping: it prevents
DESC-driven optimization studies from being mistaken for validated physical
predictions before the CBC growth rate itself is on the reference ladder.
...
norms, and the accumulated log-normalization.  The reduced artifact is written
to \path{figures/cyclone_trace_reduced.csv}.  It is designed to be compared
field by field against a matched GKW or Gyaradax run; that direct comparison is
still pending because the local Gyaradax import currently needs optional
runtime dependencies such as \texttt{omegaconf}.
...
time-history artifact for the next GKW/Gyaradax trace comparison.
...
\begin{figure}[htbp]
  ...
  amplitude, growth, norm, and normalization history used for the next
  trace-level parity check.}
