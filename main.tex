...
step is to rerun the production selected-\(k_y\) growth/profile gates with
this backend enabled.
The direct compact GKW comparison path uses
\(\texttt{finit='cosine'}\); matching the solver/Gyaradax
\(\texttt{cosine2}\) initialization would require a local GKW source patch or
a restart/state-injection path because the original GKW \texttt{finit}
selector has no native \(\texttt{cosine2}\) branch.
