...
(Sec.~\ref{sec:nonlinear-extension}) is implemented but not scientifically
...
\begin{itemize}
  ...
  \item independent nonlinear parity against a revision-pinned GX run remains
  deferred: it needs a CUDA-capable host, which was not available for this
  campaign.
\end{itemize}
...
(Sec.~\ref{sec:nonlinear-extension}) is explicitly gated behind all four
...
stencil action at roundoff.  A scratch-only native export reconstructs all 234
unfactorized stella matrices as real $12\times12$ operators with bandwidth 3.
Relative to each zero-$k_\perp$ base matrix, all 208 nonzero-$k_\perp$
corrections are diagonal and linear in $k_\perp^2$ to
$6.44\times10^{-14}$.  A mode-by-mode replay of
$\operatorname{solve}(I-\Delta t C_{\mathrm{tp}},
g_{\mathrm{in}}+\Delta t C_{\mathrm{fp}})$ reproduces all 2,808 final native
values at $1.64\times10^{-16}$ relative $L_2$.  This closes storage, sign, and
implicit-ordering ambiguity.  The local gyro-diagonal construction matches all
208 matrix differences to $6.65\times10^{-14}$ maximum absolute error.  A
differentiable block assembler also packs the native lower, diagonal, and upper
velocity blocks: 1,248 traced pair blocks reproduce all 26 zero-$k_\perp$
matrices exactly.  Independent local generation of the interior and boundary
block coefficients, followed by production residual wiring, remains before
claiming full stella parity.  Paired native runs with the mu operator enabled
and disabled localize that work: the mu path has Frobenius norm $32.2503$
($88.20\%$ of the full-block norm) and is almost entirely in the diagonal
blocks, whereas the remaining parallel-velocity path has norm $14.4030$.
Because the compact gate has only two mu nodes, both rows use boundary
closures; the mu boundary is therefore the next local coefficient target.
An additional factorial split using the mixed-coefficient knob shows that pure
mu diffusion has norm $32.2473$ ($88.19\%$ of full) and lies entirely in the
diagonal velocity blocks.  The mixed term nested in that path has norm
$0.444517$; pure parallel-velocity diffusion has norm $0.0846490$, while its
mixed path has norm $14.4028$.  This establishes the implementation order:
two-node pure-mu boundary diffusion before the parallel-path mixed derivative.
A differentiable JAX constructor implements that two-node boundary slice,
including half-mu ordered collision frequencies and both default ghost-cell
closures.  It matches all 1,248 native block rows at
$1.10\times10^{-15}$ relative $L_2$ and $7.99\times10^{-15}$ maximum absolute
error.
A second differentiable constructor now covers the mixed derivative assembled
through the parallel-velocity path, including both ghost boundaries and every
interior block.  It agrees with the native factorial component at
$9.11\times10^{-16}$ relative $L_2$ and $2.66\times10^{-15}$ maximum absolute
error.
The pure parallel-velocity half-grid flux is now constructed for arbitrary mu
grids and matches its native component at $1.35\times10^{-15}$ relative
$L_2$.  A final constructor closes the mixed-mu component at
$2.80\times10^{-15}$ relative $L_2$.  Their sum reproduces the complete native
compact block trace at $1.08\times10^{-15}$ relative $L_2$; the remaining
matrix work is general interior-$\mu$ construction and production-residual
selection.
The pure-$\mu$ constructor has also been extended to a nonuniform four-node
grid.  A new pinned trace exercises both interior rows and agrees at
$1.42\times10^{-15}$ relative $L_2$, leaving only the general mixed branches
at the coefficient layer.  Those branches now agree at $1.30\times10^{-15}$
and $6.48\times10^{-16}$ relative $L_2$; all four components reproduce the
complete 4,992-row native trace at $9.62\times10^{-16}$ relative $L_2$.
The production builder packages these blocks with the implicit identity and
mode-dependent gyro diagonal, matching the pinned zero-$k_\perp$ matrix at
$6.39\times10^{-17}$ relative $L_2$.  A complete precompute combines this
matrix with the local Laguerre--Legendre factors and exposes the resulting
backward-Euler collision solve as a selectable split step.
The four ordered species-channel matrices reconstruct the all-channel matrix at
$3.77\times10^{-17}$ relative $L_2$.  Their departure-from-identity Frobenius
norms show that the remaining coefficient comparison is dominated by the
electron--electron and electron--ion target blocks.
The underlying analytic coefficient layer is now constructed independently
from velocity nodes, magnetic field, species masses, and pair frequencies.  A
624-row native trace of speed, Maxwellian, parallel diffusion, deflection, and
mixed diffusion agrees with the differentiable JAX construction at relative
$L_2$ errors no larger than $1.99\times10^{-16}$.  Consequently the remaining
matrix gap is confined to finite-difference interior and boundary assembly and
the already isolated diagonal gyro term.
The local gyro-diagonal construction subsequently agrees with all 208
nonzero-$k_\perp$ native matrix differences to a maximum absolute error of
$6.65\times10^{-14}$.  Thus only the zero-$k_\perp$ finite-difference interior
and boundary assembly remains unresolved in the differential matrix.
