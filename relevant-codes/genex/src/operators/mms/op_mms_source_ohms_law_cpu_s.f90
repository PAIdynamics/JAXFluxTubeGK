submodule (op_mms_source_m) op_mms_source_ohms_law_cpu_s
...
contains
    ...
    module subroutine apply_ohms_law_cpu(this, t, lb, ub, lb_stripped, &
        ...
        this%n_iterations = size(b_ohms_law, kind=INT64)
    ...
    subroutine mms_source(this, t, lb, ub, lb_stripped, &
        ...
        real(kind=GP) :: omega, R, phi, Z, Lref, rhoref, q, betaref, &
                         mms_source_e_par, rhomin, rhomax, minorr, shear, &
                         absB, dabsBdR, dabsBdphi, dabsBdZ, bR, bphi, bZ, &
                         dbRdphi, dbRdZ, dbphidR, dbphidZ, dbZdR, dbZdphi, &
                         ellax1, ellax2
        ...
        !$omp              ellax1, ellax2) &
        !$omp shared(this, b_ohms_law, is_compute) &
        !$omp private(k, i, R, Z, phi, &
        !$omp         mms_source_e_par, absB, dabsBdR, &
        !$omp         dabsBdphi, dabsBdZ, bR, bphi, &
        !$omp         bZ, dbRdphi, dbRdZ, &
...
include "../../mms/slab/mms_source_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
...
include "../../mms/circular/mms_source_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
...
include "../../mms/dommaschk/mms_source_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
...
include "../../mms/salpha/mms_source_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
    ...
    subroutine mms_source_vspec(this, t, lb, ub, lb_stripped, &
        ...
        !$omp              rhomin, rhomax, minorr, shear, betaref) &
        !$omp shared(this, b_ohms_law, is_compute) &
        !$omp private(k, i, R, Z, phi, mms_source_e_par, &
        !$omp         absB, dabsBdR, &
        !$omp         dabsBdphi, dabsBdZ, bR, &
        !$omp         bphi, bZ, dbRdphi, dbRdZ, &
        !$omp         dbphidR, dbphidZ, dbZdR, dbZdphi)
        do k = lb_stripped(2), ub_stripped(2)
            if(this%mesh%equi_type() == SLAB) then
                !$omp do simd schedule(static)
...
include "../../mms/slab/mms_magfield.txt"
include "../../mms/slab/mms_source_vspec_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
...
include "../../mms/circular/mms_magfield.txt"
include "../../mms/circular/mms_source_vspec_e_par.txt"
                        b_ohms_law(i, k) = b_ohms_law(i, k) &
                                         + mms_source_e_par &
                                         * is_compute(i, k)
                enddo
...
include "../../mms/salpha/mms_magfield.txt"
include "../../mms/salpha/mms_source_vspec_e_par.txt"
                    b_ohms_law(i, k) = b_ohms_law(i, k) &
                                     + mms_source_e_par &
                                     * is_compute(i, k)
                enddo
