"""Audit odd-vpar RHS sign variants against patched GKW ``cosin2`` slices.

Run with:

    uv run --extra dev python examples/audit_cyclone_cosin2_vpar_odd_signs.py
"""
...
from jax_fluxtube_gk import run_cyclone_base_case_cosin2_vpar_odd_sign_audit
...
def main() -> None:
    ...
    with args.output.open("w", newline="") as handle:
        ...
        writer.writeheader()
        for index, name in enumerate(audit.rhs_variant_names):
            writer.writerow(
                {
                    "rhs_variant": name,
    ...
    print(
        "OPEN: "
        f"best_direct={audit.rhs_variant_names[best_direct]}, "
        ...
        f"field_flip_direct_max_abs_error={float(audit.field_flip_direct_max_abs_error):.8e}, "
        f"direct_improvement_factor={float(audit.direct_improvement_factor):.6f}"
    )
    print(args.output)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit controlled odd-vpar RHS sign variants against GKW distr*.dat."
    )
    parser.add_argument(
        "--gkw-distr1",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("figures/gkw_cosin2_cyclone_vpar_odd_sign_audit.csv"),
    )
