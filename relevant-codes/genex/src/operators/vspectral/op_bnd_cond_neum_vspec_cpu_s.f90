submodule(op_bnd_cond_m) op_bnd_cond_neum_vspec_cpu_s
    ...
    use params_bnd_cond_m, only: get_freeze_dens, &
                                 get_freeze_even_mom
    implicit none
...
contains
    ...
    module subroutine initialize_neum_vspec_cpu(this, dcomm_handler, mesh)
        ...
        if(.not. get_use_vspectral()) then
            call handle_error_vspec(VSPEC_ERR_INITIALIZE, __LINE__, __FILE__)
        end if

        this%freeze_dens = get_freeze_dens()
        this%freeze_even_mom = get_freeze_even_mom()
    ...
    module subroutine apply_neum_vspec_cpu(this, da_f_inout, da_co_qn_eq, &
        ...
        do m = lb_stripped(4), ub_stripped(4)
        do l = lb_stripped(3), ub_stripped(3)
            if(this%freeze_even_mom &
                .and. mod(this%vp(l), 2.0_GP) == 0.0_GP) cycle
            if(this%freeze_dens &
                .and. (this%vp(l) == 0.0_GP &
                .and. this%mu(m) == 0.0_GP)) cycle
        do k = lb_stripped(2), ub_stripped(2)
        !$omp do schedule(static)
