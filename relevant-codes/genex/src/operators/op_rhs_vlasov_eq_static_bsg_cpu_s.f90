submodule (op_rhs_vlasov_eq_static_m) op_rhs_vlasov_eq_static_bsg_cpu_s
    use, intrinsic :: iso_fortran_env
    use bsg_types_m, only: vspace_bsg_t, unpack_all_flags, unpack_bflag
    ! From PARALLAX
...
contains
    ...
    module subroutine apply_bsg_cpu(this, da_f_in, da_phi_in, da_A_par_in, &
        ...
        integer :: nb_point
        ! Block number of a RZ point
        logical, dimension(5) :: unpacked_flags
        ! Flags for the map matrices
        integer :: nb_neigh
        ! Block number of a neighbor RZ point
        logical :: not_block_neighbor
        ! Sign of the block number
        ...
        real(kind=GP) :: f_in_ngb_m, f_in_ngb_p, f_in_ngb_mm, f_in_ngb_pp, &
                   f_in_ngb_mp, f_in_ngb_pm, f_in_psi
        ! f values at neighbor indices
        real(kind=GP) :: vpl
        ! Local value of vp
        real(kind=GP), contiguous, dimension(:), pointer :: prefac_cd_vp_bsg
        ! Vp Prefactor for BSG
        ...
        !$omp shared(this, f_in, phi_in, f_out, absB, curl_normb_y, mu, &
        ...
        !$omp        is_compute, A_par_in, B_par_in, bsg_flags, vp_bsg, &
        !$omp        prefac_cd_vp_bsg) &
        ...
        !$omp         f_in_ngb_mm, f_in_ngb_m, f_in_ngb_p, f_in_ngb_pp, &
        !$omp         f_in_ngb_pm, f_in_ngb_mp, f_in_psi, nb_neigh, &
        !$omp         not_block_neighbor, nb_point, unpacked_flags, vpl)
        ...
        do itotal = 1, ntotal
            ...
            unpacked_flags = unpack_all_flags(bsg_flags(i))
            not_block_neighbor = unpacked_flags(5)
            nb_point = unpack_bflag(bsg_flags(i))
            vpl = vp_bsg(nb_point)%array(l)

            ! Interpolate on planes k - 2, k - 1, k + 1 and k + 2
            ! to be used for calculating y derivatives
            if (unpacked_flags(4)) then
                f_mm = dot_product( &
                      map_mm(k)%val(map_mm(k)%i(i):map_mm(k)%i(i + 1) - 1), &
                      f_in(map_mm(k)%j(map_mm(k)%i(i):map_mm(k)%i(i + 1) - 1), &
                      k - 2, l, m, n))
            else
                call this%bsg_op%dot_product_bsg( &
                        map_mm(k)%val(map_mm(k)%i(i):map_mm(k)%i(i + 1) - 1), &
                        map_mm(k)%j(map_mm(k)%i(i):map_mm(k)%i(i + 1) - 1), &
                        lb(1), lb(2), lb(3), f_in(:, :, :, m, n), nb_point, &
                        k - 2, l, f_mm)
            end if
            if (unpacked_flags(3)) then
                f_m = dot_product( &
                ...
                call this%bsg_op%dot_product_bsg( &
                        map_m(k)%val(map_m(k)%i(i):map_m(k)%i(i + 1) - 1), &
                        map_m(k)%j(map_m(k)%i(i):map_m(k)%i(i + 1) - 1), &
                        lb(1), lb(2), lb(3), f_in(:, :, :, m, n), nb_point, &
                        k - 1, l, f_m)
            end if
            if (unpacked_flags(2)) then
                f_p = dot_product( &
                ...
                call this%bsg_op%dot_product_bsg( &
                        map_p(k)%val(map_p(k)%i(i):map_p(k)%i(i + 1) - 1), &
                        map_p(k)%j(map_p(k)%i(i):map_p(k)%i(i + 1) - 1), &
                        lb(1), lb(2), lb(3), f_in(:, :, :, m, n), nb_point, &
                        k + 1, l, f_p)
            end if
            if (unpacked_flags(1)) then
                f_pp = dot_product( &
                ...
                call this%bsg_op%dot_product_bsg( &
                        map_pp(k)%val(map_pp(k)%i(i):map_pp(k)%i(i + 1) - 1), &
                        map_pp(k)%j(map_pp(k)%i(i):map_pp(k)%i(i + 1) - 1), &
                        lb(1), lb(2), lb(3), f_in(:, :, :, m, n), nb_point, &
                        k + 2, l, f_pp)
            end if
            ...
            f_in_ngb_mm = f_in(ngb_mm, k, l, m, n)
            ngb_m  = neighbors(-1,  0, i, k)
            f_in_ngb_m = f_in(ngb_m, k, l, m, n)
            ngb_mp = neighbors(-1,  1, i, k)
            f_in_ngb_mp = f_in(ngb_mp, k, l, m, n)
            ngb_pm = neighbors( 1, -1, i, k)
            f_in_ngb_pm = f_in(ngb_pm, k, l, m, n)
            ngb_p  = neighbors( 1,  0, i, k)
            f_in_ngb_p = f_in(ngb_p, k, l, m, n)
            ngb_pp = neighbors( 2,  0, i, k)
            f_in_ngb_pp = f_in(ngb_pp, k, l, m, n)
            if (not_block_neighbor) then
                nb_neigh = unpack_bflag(bsg_flags(ngb_mm))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_mm, k, l, &
                                                      f_in(:, :, :, m, n), &
                                                      lb(1), lb(2), lb(3), &
                                                      f_in_ngb_mm)
                ...
                nb_neigh = unpack_bflag(bsg_flags(ngb_m))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_m, k, l, &
                                                      f_in(:, :, :, m, n), &
                ...
                nb_neigh = unpack_bflag(bsg_flags(ngb_mp))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_mp, k, l, &
                                                      f_in(:, :, :, m, n), &
                ...
                nb_neigh = unpack_bflag(bsg_flags(ngb_pm))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_pm, k, l, &
                                                      f_in(:, :, :, m, n), &
                ...
                nb_neigh = unpack_bflag(bsg_flags(ngb_p))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_p, k, l, &
                                                      f_in(:, :, :, m, n), &
                ...
                nb_neigh = unpack_bflag(bsg_flags(ngb_pp))
                if (nb_neigh /= nb_point) then
                    call this%bsg_op%interpolate_dist(nb_point, nb_neigh, &
                                                      ngb_pp, k, l, &
                                                      f_in(:, :, :, m, n), &
            ...
            dfdx = this%prefac_cd_xz &
                 * (- 1.0_GP / 12.0_GP * f_in_ngb_pp &
                    + 2.0_GP /  3.0_GP * f_in_ngb_p &
                    - 2.0_GP /  3.0_GP * f_in_ngb_m &
                    + 1.0_GP / 12.0_GP * f_in_ngb_mm)
            ...
            bps = absB(i, k) + this%prefac_bps(n) * vpl * curl_normb_y(i, k)
            ...
            do ox = -1, 1
                ...
                zeta(ox, oz) = &
                    ...
                    - this%prefac_flutter_xyz(n) * vpl &
                                                 * A_par_in(ngb_m, k) &
                    + this%masses(n)             * this%H2(ngb_m, k)

                f_in_psi = f_in(ngb_m, k, l, m, n)
                if (not_block_neighbor) then
                    nb_neigh = unpack_bflag(bsg_flags(ngb_m))
                    if (nb_neigh /= nb_point) then
                        call this%bsg_op%interpolate_dist(nb_point, &
                                                          nb_neigh, &
                                                          ngb_m, k, l, &
                                                          f_in(:, :, :, m, n), &
                                                          lb(1), lb(2), lb(3), &
            ...
            pb = pb &
               - dfdz * (-mu(m) * this%temp_scalings(n) * dabsBdx(i, k) &
                         + gyx(i, k) &
                            * (grad_H_y &
                                - this%prefac_flutter_xyz(n) * vpl * dAdy)) &
               ...
               - dfdy * (- gyx(i, k) &
                            * (grad_H_z &
                                - this%prefac_flutter_xyz(n) * vpl * dAdz) &
                         + gyz(i, k) &
                            * (grad_H_x &
                                - this%prefac_flutter_xyz(n) * vpl * dAdx))
            ...
            ! A.12 (a) 1st line
            bs_adv = -2.0_GP * this%prefac_par(n)* vpl * absB(i, k) * dfdy
            ! Contribution of the curvature term
            ! A.12 (a) 2nd line
            bs_adv = bs_adv + 2.0_GP * vpl * this%prefac_orth(n) * vpl &
            ...
            vp_adv = vp_adv - this%prefac_orth(n) * vpl &
