submodule (timestep_m) timestep_init_vspec_cpu_s
    !! Contains implementation of the initialization of
    !! spectral-method-supported operators for timestep_t
    ...
    use op_diag_mom_0d_m, only: op_diag_mom_0d_vspec_t, &
                                op_diag_mom_0d_vspec_cpu_t
    use op_diag_mom_2d_m, only: op_diag_mom_2d_vspec_t, &
                                op_diag_mom_2d_vspec_cpu_t
    use op_rhs_vlasov_eq_static_m, only: op_rhs_vlasov_eq_static_vspec_cpu_t
    use op_rhs_vlasov_eq_dynamic_m, only: op_rhs_vlasov_eq_dynamic_vspec_cpu_t
    use op_mom_maxwells_eq_m, only: op_mom_maxwells_eq_vspec_cpu_t
    use op_mom_ohms_law_m, only: op_mom_ohms_law_vspec_cpu_t
    use op_coll_m, only: op_coll_t, op_coll_vspec_t, op_coll_lbd_vspec_cpu_t
    use op_mom_coll_m, only: op_mom_coll_vspec_cpu_t
    use op_source_m, only: op_source_loc_dens_vspec_cpu_t, &
...
contains
    ...
    module subroutine initialize_operators_vspec_cpu(this)
        ...
        allocate(op_diag_mom_0d_vspec_cpu_t :: this%op_mom_0d)
        allocate(op_diag_mom_2d_vspec_cpu_t :: this%op_mom_2d)
        ...
        allocate(op_mom_maxwells_eq_vspec_cpu_t :: this%op_mom_maxwells_eq)
        allocate(op_mom_ohms_law_vspec_cpu_t :: this%op_mom_ohms_law)
        ...
        select type(op => this%op_mom_ohms_law)
            type is(op_mom_ohms_law_vspec_cpu_t)
                call op%initialize(this%dcomm_handler, this%mesh)
        end select
        ...
        select case(coll_type)
            case("none") ! No collisions
                this%apply_collisions = .false.

            case("lbd") ! LBD collision operator
                allocate(op_coll_lbd_vspec_cpu_t :: this%op_coll)
        ...
        if(this%apply_collisions) then
            ...
            allocate(op_mom_coll_vspec_cpu_t :: this%op_mom_coll)
        ...
        if(this%apply_sources) then
            ...
            do s = 1, this%n_source
                ...
                select case(source_type)
                    case("dens")
                        allocate(op_source_loc_dens_vspec_cpu_t :: &
                                 this%op_source_container(s)%op_source)
                    case("torque")
                        allocate(op_source_loc_torque_vspec_cpu_t :: &
                                 this%op_source_container(s)%op_source)
                    case("heat")
                        allocate(op_source_loc_heat_vspec_cpu_t :: &
                                 this%op_source_container(s)%op_source)
                end select
        ...
        if(this%run_mms) then
            ...
            select case(bnd_cond_type)
                ...
                case('neumann')
                    allocate(op_bnd_cond_neum_vspec_cpu_t :: this%op_bnd_cond)
                case default
                    call handle_error(&
