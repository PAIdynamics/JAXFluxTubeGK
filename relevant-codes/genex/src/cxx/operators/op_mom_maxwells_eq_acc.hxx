#ifndef OP_MOM_MAXWELLS_EQ_ACC_HXX
#define OP_MOM_MAXWELLS_EQ_ACC_HXX
...
// with OpenMP on CPU
class op_mom_maxwells_eq_acc_t: public op_mom_maxwells_eq_gpu_t
{
public:
    // Constructor of the OpenMP child class
    op_mom_maxwells_eq_acc_t(const mesh_5d_t& mesh)
    ...
    ~op_mom_maxwells_eq_acc_t() override
    {
        // Deallocate the device pointers of the class members from the device
        #pragma acc exit data \
            delete(this->prefac_b_bpar_ptr[:this->size_sp], \
                   this->prefac_co_qn_ptr[:this->size_sp], \
                   this->prefac_b_qn_ptr[:this->size_sp], \
                   this->prefac_b_amp_ptr[:this->size_sp], \
                   this->prefac_bps_ptr[:this->size_sp], \
    ...
    op_mom_maxwells_eq_acc_t(const op_mom_maxwells_eq_acc_t&) = delete;

    // Copy-assignment operator is disabled
    op_mom_maxwells_eq_acc_t& operator=(const op_mom_maxwells_eq_acc_t&)
    = delete;
                  ...
                  data_array_t<real_t, 2>& b_bpar_eq) const override
    {
        ...
        data_array_acc_t<real_t, 3> send_buffer({lb[0], lb_stripped[1], 1},
                                                {ub[0], ub_stripped[1], 4},
                                                0.0);
        data_array_acc_t<real_t, 3> receive_buffer({lb[0], lb_stripped[1], 1},
                                                   {ub[0], ub_stripped[1], 4});
        ...
        #pragma acc parallel default(none) \
            present(lb, ub, lb_stripped, ub_stripped, \
                    this, mesh, co_qn_eq, b_qn_eq, \
                    b_amps_law, b_bpar_eq, receive_buffer)
        #pragma acc loop independent gang vector collapse(2)
        for (int32_t k = lb_stripped[1]; k <= ub_stripped[1]; k++)
        for (int32_t i = lb[0]; i <= ub[0]; i++)
        {
