...
def create_slurm_script_gpu(build_dir, job_config, gpu_job_info, runtime_modes,
                            geom, res):
    job_name         = job_config["general"]["job_name"]
    output_file      = job_config["general"]["output_file"]
    partition_type   = gpu_job_info["partition_type"]
    partition_name   = gpu_job_info["partition_name"]
    qos              = gpu_job_info["qos"]
    n_nodes          = gpu_job_info["n_nodes"]
    ntasks_per_node  = gpu_job_info["ntasks_per_node"]
    time_limit       = gpu_job_info["time_limit"]
    cpus_per_task    = gpu_job_info["cpus_per_task"]
    slurm_gpu_gres   = gpu_job_info["slurm_gpu_gres"]
    memory           = gpu_job_info["memory"]
    is_exclusive     = gpu_job_info["is_exclusive"]
    slurm_constraint = gpu_job_info["slurm_constraint"]
    ...
    cpu_fallback = False
    if gpu_modes == ["cxx"] or slurm_gpu_gres == "none":
        cpu_fallback = True
    ...
    script.append('#SBATCH -o mms_gpu.out')
    ...
    # invalid batch file.
    if gpu_job_info["partition_as_constraint"]:
    ...
    for mode in gpu_modes:
        for line in job_config["general"]["execution_commands"]:
            ...
            sline = sline.replace("<OUT_DIR>", "./" + mode + "/")
            sline = sline.replace("<PARAMS_IN_FILE>", "./" + mode + \
                                                      "/params_in.txt")
            sline += " 2>&1 |& tee ./" + mode + "/" + output_file

            script.append("( cd " + mode + "; " \
