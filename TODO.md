...
- compare a richer GKW selected-`ky` amplitude or phi history against the now-passing Gyaradax/`CycloneTrace` histories, because compact `time.dat` exposes a remaining \(0.02\)-level growth-history gap but not the field normalization source;
- add a GKW-compatible `cosine2` initialization or restart/state-injection path, or otherwise document why GKW's native `cosine` initialization is the only direct `time.dat` path;
