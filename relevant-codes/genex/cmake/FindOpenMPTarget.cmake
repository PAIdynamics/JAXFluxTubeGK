...
function(_OPENMP_TARGET_DEVICE_ARCH_CANDIDATES LANG DEVICE DEVICE_FLAG)
  if(OpenMPTarget_${DEVICE}_ARCH)
    ...
    set(OMPTarget_ARCH_Clang "--offload-arch=${OpenMPTarget_${DEVICE}_ARCH}")
...
function(_OPENMP_TARGET_DEVICE_GET_FLAGS LANG DEVICE OPENMP_FLAG_VAR OPENMP_LIB_VAR OPENMP_DEVICE_VAR OPENMP_ARCH_VAR)
  ...
  message(STATUS "Calling _OPENMP_TARGET_DEVICE_CANDIDATES for ${LANG} and ${DEVICE}")
  _OPENMP_TARGET_DEVICE_CANDIDATES(${LANG} ${DEVICE})

  message(STATUS "Calling _OPENMP_TARGET_DEVICE_FLAG_CANDIDATES for ${LANG} and ${OpenMPTarget_${LANG}_DEVICE_CANDIDATES}")
  _OPENMP_TARGET_DEVICE_FLAG_CANDIDATES(${LANG} "${OpenMPTarget_${LANG}_DEVICE_CANDIDATES}")

  message(STATUS "Calling _OPENMP_TARGET_DEVICE_ARCH_CANDIDATES(${LANG} ${DEVICE} ${OpenMPTarget_${LANG}_DEVICE_CANDIDATES})")
  _OPENMP_TARGET_DEVICE_ARCH_CANDIDATES(${LANG} ${DEVICE} "${OpenMPTarget_${LANG}_DEVICE_CANDIDATES}")

  #message(STATUS "Calling _OPENMP_TARGET_WRITE_SOURCE_FILE(${LANG} TEST_SOURCE ...)")
  _OPENMP_TARGET_WRITE_SOURCE_FILE("${LANG}" "TEST_SOURCE" OpenMPTargetTryFlag _OPENMP_TEST_SRC)
  #message(STATUS "_OPENMP_TEST_SRC=${_OPENMP_TEST_SRC}")

  # Try to compile a test application with the found flags.
  #message(STATUS "CMAKE_FLAGS=-DCOMPILE_DEFINITIONS:STRING=${OpenMP_${LANG}_FLAGS} ${OpenMPTarget_${LANG}_FLAG_CANDIDATES} ${OpenMPTarget_${LANG}_DEVICE_ARCH_CANDIDATES} -DINCLUDE_DIRECTORIES:STRING=${OpenMP_${LANG}_INCLUDE_DIR}")
  ...
  #message(STATUS "OUTPUT_VARIABLE is ${OpenMP_TRY_COMPILE_OUTPUT}")
  ...
  if (OpenMPTarget_COMPILE_RESULT AND DEFINED OpenMPTarget_${LANG}_DEVICE_CANDIDATES)
    ...
    if(CMAKE_${LANG}_COMPILER_ID STREQUAL "Clang")
      find_library(OpenMPTarget_libomptarget_LIBRARY
        ...
        ${OpenMPTarget_ROOT} ${OPENMPTARGET_ROOT}
      )
      message(STATUS "OpenMPTarget_libomptarget_LIBRARY=${OpenMPTarget_libomptarget_LIBRARY}")
...
foreach(LANG IN ITEMS CXX)
  ...
  foreach(DEVICE IN ITEMS AMDGPU)
    if(CMAKE_${LANG}_COMPILER_LOADED)
      message(STATUS " ===== Working for LANG=${LANG} and DEVICE=${DEVICE}")
      if(NOT DEFINED OpenMPTarget_${LANG}_FLAGS OR NOT DEFINED OpenMPTarget_${LANG}_DEVICE)
        ...
        message(STATUS "_OpenMPTarget_${DEVICE}_FLAGS=${_OpenMPTarget_${DEVICE}_FLAGS}")
        ...
        message(STATUS "OpenMPTarget_${DEVICE}_FLAGS=${OpenMPTarget_${DEVICE}_FLAGS}")
        ...
        message(STATUS "OpenMPTarget_${DEVICE}_ARCH=${OpenMPTarget_${DEVICE}_ARCH}")
        message(STATUS "OpenMPTarget_${DEVICE}_LIBS_WORK=${OpenMPTarget_${DEVICE}_LIBS_WORK}")
...
#message(STATUS "CMAKE_Fortran_COMPILER_LOADED=${CMAKE_Fortran_COMPILER_LOADED}")
