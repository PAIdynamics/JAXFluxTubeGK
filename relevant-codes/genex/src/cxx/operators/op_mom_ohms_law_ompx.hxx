#ifndef OP_MOM_OHMS_LAW_OMPX_HXX
#define OP_MOM_OHMS_LAW_OMPX_HXX
...
// with OpenMP offload
class op_mom_ohms_law_ompx_t: public op_mom_ohms_law_gpu_t
{
public:
    // Constructor of the OpenMP offload child class
    op_mom_ohms_law_ompx_t(const mesh_5d_t& mesh)
    ...
    ~op_mom_ohms_law_ompx_t() override
    {
        // Deallocate the device pointers of the class members from the device
        #pragma omp target exit data \
            map(delete: this->prefac_norm_b_ptr[:this->size_sp], \
                        this->prefac_norm_lambda_ptr[:this->size_sp], \
                        this->prefac_bps_ptr[:this->size_sp], \
                        this->prefac_cd_vp, \
                        this->PI, \
    ...
    op_mom_ohms_law_ompx_t(const op_mom_ohms_law_ompx_t&) = delete;

    // Copy-assignment operator is disabled
    op_mom_ohms_law_ompx_t& operator=(const op_mom_ohms_law_ompx_t&) = delete;
        ...
        data_array_t<real_t, 2>& b_ohms_law) const override
    {
        ...
        data_array_ompx_t<real_t, 3> send_buffer(
            {lb_stripped[0], lb_stripped[1], 1},
            {ub_stripped[0], ub_stripped[1], 2}, 0.0);
        data_array_ompx_t<real_t, 3> receive_buffer(
            {lb_stripped[0], lb_stripped[1], 1},
        ...
        #pragma omp target teams distribute parallel for simd collapse(2) \
            ...
            shared(lb_stripped, ub_stripped, mesh, f_in, dfdt_in, send_buffer)
        ...
        for (int32_t i = lb_stripped[0]; i <= ub_stripped[0]; i++)
        {
            ...
            {
                this->comp_kernel_prior(i, k, l, m, n, mesh,
                                        f_in, dfdt_in, send_buffer);
            }
        }
        ...
        #pragma omp target teams distribute parallel for simd collapse(2) \
            ...
            shared(lb_stripped, ub_stripped, \
                   mesh, lambda_ohms_law, b_ohms_law, receive_buffer)
        for (int32_t k = lb_stripped[1]; k <= ub_stripped[1]; k++)
        for (int32_t i = lb_stripped[0]; i <= ub_stripped[0]; i++)
        {
