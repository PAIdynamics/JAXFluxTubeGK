...
- [ ] Diagnose the remaining `cosine2`/`cosin2` growth and parallel-profile gap after the initialization mismatch is removed.
...
- use the patched GKW `cosin2` fixtures as the primary selected-`ky` comparison while keeping the native `finit='cosine'` fixtures as a cross-check;
- diagnose the remaining `cosine2`/`cosin2` residual gap: late-fit growth is low by \(2.35\times10^{-2}\), late-window mean is low by \(2.18\times10^{-2}\), and the row-normalized parallel-profile maximum error is \(2.96\times10^{-2}\);
- inspect any remaining production GKW conventions that affect the evolved mode history rather than the already-passing source-level term, RK4, normalization, field-packing, matrix-format, and initialization checks;
