...
reduced `stellarator_gk` regression diagnostics pending solver-side parity
against the matched stella fixture. The external GX replacement workflow is also
...
as `examples/run_w7x_mode_structure_gate.py`; it can report a
machine-readable `PENDING` state when the external fixture is absent and now
compares the exported matched stella fixture in this repository. The W7-X production
readiness ledger is now generated by
`scripts/run_w7x_production_readiness_gate.py`; the committed result passes the
reduced convergence regression and remains
`blocked_external_mode_structure_parity` until solver/stella mode-structure
parity and true production CPU timing pass. The
guarded CPU-timing writer `scripts/run_w7x_production_cpu_timing.py` now writes
the production-control timing contract and records the current
...
W7-X `.big.nc`/`.out.nc` outputs are present. The run directory now includes
`run_external_reference.sh` for the external GX-capable machine and
`ingest_returned_outputs.sh` for repository-side returned-output ingestion, plus
`run_production_timing_after_parity.sh` for the post-parity CPU timing/readiness
claim. `scripts/package_w7x_external_reference_bundle.py` now writes a transfer
tarball with a SHA-256 manifest for that external run; the local handoff bundle
has been generated at
`fixtures/gx_w7x_mode_structure_run/w7x_external_reference_bundle.tar.gz`, with
the W7-X VMEC file copied into the run directory. The refreshed workflow status
records `local_bundle_exists=true`, `vmec_destination_exists=true`, and the
same missing-GX-output blocker. `scripts/ingest_w7x_external_reference_outputs.py`
writes a separate
`blocked_missing_external_outputs` ingest ledger until the returned GX
`.big.nc`/`.out.nc` files exist. stella is now present locally as a better
CPU continuum W7-X reference candidate: its submodules were initialized, CMake
configured against Homebrew `gfortran`/`mpifort`/FFTW/NetCDF-Fortran, and the
`stella` target built successfully. The bundled stella
`LINEAR_W7X_SINGLEMODE` W7-X example ran on CPU in `/private/tmp`, producing
`input.out.nc`; `examples/export_stella_mode_structure_fixture.py` exported a
valid shared fixture with `ky=0.5`, late-window
`gamma=0.0948506358629369`, and `omega_r=-0.028097042438257974`. This is a
reference-pipeline smoke, not the production W7-X reference, because it is not
matched to the solver's current W7-X geometry/electron-model/`ky` setup. The
matched stella W7-X run directory is now prepared in
`fixtures/stella_w7x_mode_structure_run/`: it uses the GX benchmark VMEC,
`torflux=0.64`, `alpha0=0`, `nfield_periods=34.75236`, adiabatic electrons,
`ky=(0,0.1,0.2,0.3)`, `kx=0`, `nzed=256`, `nmu=8`, `nvgrid=16`, `tend=200`,
`delt=0.1`, and the late half-window growth convention. A reduced
matched-physics stella smoke ran in `/private/tmp/stella_w7x_matched_smoke`
and exported `ky=(0.1,0.2,0.3)`, confirming the input syntax, adiabatic-electron
path, W7-X VMEC use, twist-and-shift boundary path, and stella `.out.nc`
fixture schema. The matched production stella W7-X run has now completed on
one CPU in about 295 seconds and exported
`fixtures/w7x_itg_external_mode_structure_fixture.csv` with `ky=(0.1,0.2,0.3)`
and 257 normalized z points after re-export with
`--stella-z-coordinate zed_over_2pi`. The late-half-window stella values are
`gamma=(-7.797962e-04,-4.876057e-03,9.619444e-03)` and
`omega_r=(-6.573003e-02,-1.181846e-01,-2.849346e-03)`. The reduced
solver-vs-stella W7-X gate is now a real parity blocker rather than a missing
artifact: it remains open with maximum growth, frequency, and phase-aligned
profile errors `1.89858341e-01`, `1.39078602e-01`, and `3.02836095e-01`.
`scripts/audit_w7x_stella_solver_parity.py` now makes the debugging order
machine-readable: normalized-z and selected-`ky` checks pass, the first failed
check is `field_line_length` (`field_line_periods=1` versus stella
`nfield_periods/nfp \approx 6.95` zeta turns), and later blockers are
...
`blocked_missing_complex_stella_rhs_trace`. A targeted non-destructive stella
source-copy patch now closes that acquisition blocker for the explicit-term
audit path: `scripts/prepare_stella_w7x_rhs_trace_run.py` copies the local
stella tree to `/tmp`, patches only the copied
`gyrokinetic_equation_explicit.f90`, and generates a W7-X `ky=0.3`,
`iky=4`, `ikx=1` run that forces mirror and parallel streaming through
`add_explicit_gyrokinetic_terms`. The patched stella copy built and ran to
`istep=2000` on one CPU; the raw trace is intentionally left in `/tmp`
because it is 263 MB. The committed compact summary
`fixtures/w7x_ky03_stella_rhs_trace_summary/rhs_trace_summary.json` records
`required_record_terms_present=true`, `total_rows=1382403`, `code_dt=0.1`,
and nonzero explicit-term norms for mirror, magnetic-drift-y, equilibrium
drive, and parallel streaming. The next W7-X blocker is now ingestion and
array-level comparison of that stella trace against
`fixtures/w7x_ky03_rhs_model_balance/`, with attention to native `rhs*dt`
units and stella's explicit-term diagnostic setting.
The current
benchmark-informed
...
- `scripts/prepare_stella_w7x_reference_run.py`: reproducible external stella
  run-prep generator for the matched CPU continuum W7-X complex
...
- `scripts/run_w7x_reduced_convergence_study.py`: reduced W7-X convergence,
  ...
  generator.
- `scripts/run_w7x_production_readiness_gate.py`: single-command W7-X
  production-readiness ledger; it audits the committed reduced convergence
  artifacts, runs the external mode-structure gate, and blocks production DESC
  optimization until external W7-X parity and true production CPU timing pass.
- `scripts/run_w7x_production_cpu_timing.py`: guarded W7-X production-control
...
- `scripts/package_w7x_external_reference_bundle.py`: W7-X external-reference
  bundle writer for transferring the patched GX input, VMEC file, README,
  handoff scripts, and SHA-256 manifest to a GX/CUDA-capable machine.
- `scripts/ingest_w7x_external_reference_outputs.py`: returned-output ingest
  command; it accepts returned W7-X GX `.big.nc`/`.out.nc` files, optionally
  copies them into the prepared run directory, exports the portable CSV
  fixture, runs the mode-structure gate, and refreshes the production-readiness
  ledger.
- `examples/export_stella_mode_structure_fixture.py`: stella `.out.nc`
  exporter; it converts `phi_vs_t(t,tube,zed,kx,ky,ri)` and
  `omega(t,kx,ky,ri)=omega+i*gamma` into the same portable per-`ky`
  mode-structure CSV used by the W7-X gate.
...
- `examples/desc_fixture_optimization_loop.py`: reduced DESC DSHAPE
  optimization loop with a production-readiness guard for
...
- `fixtures/gx_w7x_mode_structure_run/`: patched GX W7-X input plus
  ...
  blocker, `external_reference_ingest_status.json` recording the current
  missing-returned-output blocker, and shell handoff scripts for the external
  GX run, returned-output ingest, and post-parity CPU timing.
- `fixtures/stella_w7x_mode_structure_run/`: matched stella W7-X
  adiabatic-electron production input, copied GX benchmark VMEC,
  run/export/compare shell scripts, and metadata for producing the preferred
  CPU continuum external W7-X mode-structure fixture.
- `fixtures/w7x_itg_convergence_study/`: reduced W7-X convergence/timing
  ...
  `production_readiness_gate.json`, `production_cpu_timing.json`, an external
  mode-structure gate status, and README.
...
- `relevant-codes/`: Gyaradax, DESC, GKW, GX, and stella source trees.
...
- `relevant-codes/stella/`: local stella checkout, now built on CPU and used
  as the preferred W7-X continuum reference route.
...
Relative to `task.tex`, the code base now has the first linear electrostatic
stellarator-flux-tube solver architecture, reduced stellarator examples, and
differentiable fixed-topology objectives. What remains before a trusted
stellarator simulation claim is not a missing entry point but a set of
validation gates: external W7-X mode-structure parity, production-control W7-X
convergence, production CPU timing, and a clear decision on the remaining GKW
full-state/velocity-space discrepancy. A full nonlinear turbulence stellarator
run is beyond the current implemented physics and still requires nonlinear ExB
dealiasing, nonlinear flux diagnostics, and nonlinear benchmark parity.
...
flux-tube run, then use it as the gate for production CPU timing and DESC
optimization readiness. This is the next `task.tex` milestone. A full nonlinear
turbulence stellarator run remains a later physics-extension milestone.

Priority order:

1. Ingest the explicit-term stella trace generated by
   `scripts/prepare_stella_w7x_rhs_trace_run.py` and compare it with
...
2. Keep the already available standard-output comparison as a guardrail:
   ...
   true term parity.
3. If the explicit-term trace does not isolate the mismatch, add a same-geometry
   single-`ky=0.3` solver trace with stored complex `phi(z,t)`, density moment,
   temperature/energy moment, and field-solve numerator/denominator, then add
   the smallest matching stella diagnostic hook in the implicit path.
...
- keep the passing RH plateau, selected Cyclone scalar growth, CBC algebra,
  ...
  row-normalized `parallel_phi.dat`, DESC/GX/eik geometry, W7-X eik-source,
  and reduced W7-X fixture-contract tests as regression gates;
- keep the exact-grid low-`ky` Cyclone branch-shape gap visible:
  `gamma(0.3)=0.14325556436195153` versus calibrated reference
  `0.3080402563529299`;
- do not claim full turbulence capability until nonlinear ExB/dealiasing,
  nonlinear heat-flux diagnostics, and nonlinear benchmark parity exist.

Expected file changes:

- a stella-vs-solver term-balance comparator using the explicit-term trace
  summary and, locally, the raw `/tmp` trace,
...
### 2026-06-11: Added Targeted stella W7-X `ky=0.3` RHS Trace Patch

- Added `scripts/prepare_stella_w7x_rhs_trace_run.py`, which prepares a
  non-destructive copied stella tree under `/tmp`, patches only the copied
  `gyrokinetic_equation_explicit.f90`, and writes build/run scripts for the
  matched W7-X `ky=0.3`, Fortran `iky=4`, `ikx=1` trace.
- The generated RHS-trace input forces `stream_implicit=F`,
  `mirror_implicit=F`, `mirror_semi_lagrange=F`, and `fully_explicit=T` so
  `add_explicit_gyrokinetic_terms` can export mirror and parallel-streaming
  deltas. The production stella mode-structure run keeps those terms implicit,
  so this is a term-balance diagnostic path rather than a production growth
  run.
- The patched copied stella source built successfully and the generated run
  reached `istep=2000` with `code_dt=0.1`. The raw trace
  `/tmp/stellarator_gk_stella_w7x_rhs_trace/run/stellarator_gk_w7x_ky03_rhs_trace.dat`
  has 1,382,403 data rows and is 263 MB, so it is not committed.
- Added `scripts/summarize_stella_w7x_rhs_trace.py` and committed the compact
  summary fixture
  `fixtures/w7x_ky03_stella_rhs_trace_summary/rhs_trace_summary.json`.
  It records `required_record_terms_present=true` for `pdf_g`, `phi`,
  mirror force, magnetic-drift-y, magnetic-drift-x, equilibrium drive,
  parallel streaming, and total RHS. The selected `kx=0` magnetic-drift-x
  record is present with zero norm, as expected.
- Added focused tests in `tests/test_stella_w7x_rhs_trace_prep.py` and
...
- Commands run:
  ...
  - `python3 -m py_compile scripts/prepare_stella_w7x_rhs_trace_run.py scripts/summarize_stella_w7x_rhs_trace.py tests/test_stella_w7x_rhs_trace_prep.py tests/test_stella_w7x_rhs_trace_summary.py`
  - `uv run ruff check scripts/prepare_stella_w7x_rhs_trace_run.py scripts/summarize_stella_w7x_rhs_trace.py tests/test_stella_w7x_rhs_trace_prep.py tests/test_stella_w7x_rhs_trace_summary.py`
  - `JAX_ENABLE_X64=1 uv run pytest tests/test_stella_w7x_rhs_trace_prep.py tests/test_stella_w7x_rhs_trace_summary.py -q`
...
- Fixed the stella fixture export convention: `examples/export_stella_mode_structure_fixture.py`
  now defaults to `zed_over_2pi`, and the generated
  `fixtures/stella_w7x_mode_structure_run/export_stella_fixture.sh` passes
  `--stella-z-coordinate zed_over_2pi` explicitly. The exported stella fixture
  now spans normalized `z=[-0.5,0.5]`, matching the solver/GX fixture
  convention.
- Re-exported `fixtures/w7x_itg_external_mode_structure_fixture.csv` from the
  existing stella `.out.nc`, reran the mode-structure comparison, and refreshed
  the W7-X production-readiness ledger. The gate remains open with
  `max_growth_error=1.89858341e-01`,
  `max_frequency_error=1.39078602e-01`, and
  `max_profile_error=3.02836095e-01`.
- Added `scripts/audit_w7x_stella_solver_parity.py`, which audits the
  solver-vs-stella mismatch in the requested order: eik/z coordinate,
  field-line length, `ky`, twist-and-shift/linking, growth-window/time,
  field normalization, then velocity/RHS terms.
- The audit writes
  `fixtures/w7x_itg_convergence_study/stella_solver_parity_audit.json`.
  Current result: normalized z and selected `ky` pass; the first failed check
  is `field_line_length`, because the committed reduced solver fixture uses
  `field_line_periods=1` while stella spans
  `nfield_periods/nfp \approx 6.95` zeta turns. Later blockers are
  `n_kx=3`/`ikxspace=2` versus stella `nakx=1`, and solver total time
  `0.012` versus stella `tend=200`.
- Commands run:
  ...
  - `uv run python examples/compare_mode_structure_fixtures.py --observed fixtures/w7x_itg_reduced_benchmark/mode_structures.csv --reference fixtures/w7x_itg_external_mode_structure_fixture.csv --ky-values 0.1,0.2,0.3 --require-profile --resample-reference-to-observed-z --output figures/w7x_itg_external_mode_structure_comparison.csv`
  - `uv run python scripts/run_w7x_production_readiness_gate.py`
  - `uv run python scripts/audit_w7x_stella_solver_parity.py`

### 2026-06-11: Ran Matched stella W7-X Reference and Opened Parity Gate

- Ran the matched stella W7-X production input from
  `fixtures/stella_w7x_mode_structure_run/` on CPU. stella used the GX
  benchmark VMEC, `torflux=0.64`, `alpha0=0`, `nfield_periods=34.75236`,
  one kinetic ion species, adiabatic electrons with
  `field-line-average-term`, `ky=(0,0.1,0.2,0.3)`, `nzed=256`, `nmu=8`,
  `nvgrid=16`, `tend=200`, and `delt=0.1`.
- Exported `fixtures/w7x_itg_external_mode_structure_fixture.csv` from the
  resulting `stella_w7x_adiabatic_electrons.out.nc`. The fixture has 771 rows:
  257 z points for each of `ky=(0.1,0.2,0.3)`. Late-half-window stella values
  are `gamma=(-7.797962e-04,-4.876057e-03,9.619444e-03)` and
  `omega_r=(-6.573003e-02,-1.181846e-01,-2.849346e-03)`.
- Extended `examples/compare_mode_structure_fixtures.py` with explicit
  `--resample-reference-to-observed-z` and
  `--resample-observed-to-reference-z` options so high-resolution external
  fixtures can be compared to the committed reduced solver fixture without
  pretending the z grids are identical.
- Updated `scripts/run_w7x_production_readiness_gate.py` to call the W7-X
  external mode-structure gate with reference-to-observed z resampling and to
  distinguish a missing external fixture from an existing-but-open parity
  fixture. The current production readiness status is now
  `blocked_external_mode_structure_parity`.
- Refreshed
  `fixtures/w7x_itg_convergence_study/external_mode_structure_gate/gate_status.json`
  and `production_readiness_gate.json`. The reduced convergence regression
  passes, but the stella parity gate is open with maximum growth, frequency,
  and phase-aligned profile errors `1.89858341e-01`,
  `1.39078602e-01`, and `3.01086637e-01`.
- Commands run:
  - `bash fixtures/stella_w7x_mode_structure_run/run_stella_reference.sh`
  - `uv run python examples/export_stella_mode_structure_fixture.py --stella-output fixtures/stella_w7x_mode_structure_run/stella_w7x_adiabatic_electrons.out.nc --ky-values 0.1,0.2,0.3 --average-fraction 0.5 --output fixtures/w7x_itg_external_mode_structure_fixture.csv`
  - `uv run python scripts/prepare_stella_w7x_reference_run.py --overwrite`
  - `uv run python examples/compare_mode_structure_fixtures.py --observed fixtures/w7x_itg_reduced_benchmark/mode_structures.csv --reference fixtures/w7x_itg_external_mode_structure_fixture.csv --ky-values 0.1,0.2,0.3 --require-profile --resample-reference-to-observed-z --output figures/w7x_itg_external_mode_structure_comparison.csv`
  - `uv run python scripts/run_w7x_production_readiness_gate.py`

### 2026-06-10: Added stella CPU W7-X Reference Path

- Inspected the new `relevant-codes/stella` checkout and found bundled W7-X
  examples in `POST_PROCESSING/stellapy/examples/LINEAR_W7X_SINGLEMODE`,
  `LINEAR_W7X_RANGE`, and `NONLINEAR_W7X`, plus VMEC equilibria under
  `DOCUMENTATION/VMEC_equilibria`.
- Added `load_stella_mode_structure_fixture`, which reads stella `.out.nc`
  `phi_vs_t(t,tube,zed,kx,ky,ri)` and
  `omega(t,kx,ky,ri)=omega+i*gamma` into the shared
  `PerKyModeStructureFixture` contract.
- Added `examples/export_stella_mode_structure_fixture.py` so a stella W7-X
  run can directly generate
  `fixtures/w7x_itg_external_mode_structure_fixture.csv`; requested `ky`
  values are checked against the stella output with an explicit tolerance so a
  benchmark run cannot silently compare the wrong mode.
- Initialized stella submodules after the sandboxed attempt failed on DNS, then
  configured and built stella with CMake against the local CPU toolchain.
- Ran the bundled stella `LINEAR_W7X_SINGLEMODE` W7-X example on CPU in
  `/private/tmp/stella_w7x_reference_smoke` and exported a valid `ky=0.5`
  fixture. The smoke values are late-window
  `gamma=0.0948506358629369` and `omega_r=-0.028097042438257974`.
- Updated `TODO.md` and `main.tex` so stella is the preferred W7-X
  physics-reference route, while GX remains an optional GPU/moment-method
  cross-check.
- Commands run:
  - `git -C relevant-codes/stella submodule update --init --recursive`
  - `cmake . -B COMPILATION/build_cmake -DnetCDFFortran_ROOT=/opt/homebrew -DFFTW_ROOT=/opt/homebrew`
  - `cmake --build COMPILATION/build_cmake --target stella -j 4`
  - `/Users/mohsensadr/Codes/GitHub/new-plasma-code/relevant-codes/stella/stella input.in`
  - `uv run python examples/export_stella_mode_structure_fixture.py --stella-output /private/tmp/stella_w7x_reference_smoke/input.out.nc --ky-values 0.5 --output /private/tmp/stella_w7x_reference_smoke/stella_w7x_mode_structure_fixture.csv`
  - `uv run python examples/compare_mode_structure_fixtures.py --observed /private/tmp/stella_w7x_reference_smoke/stella_w7x_mode_structure_fixture.csv --reference /private/tmp/stella_w7x_reference_smoke/stella_w7x_mode_structure_fixture.csv --ky-values 0.5 --require-profile --output /private/tmp/stella_w7x_reference_smoke/stella_self_compare.csv`
  - `uv run ruff check src/stellarator_gk/benchmarks.py src/stellarator_gk/__init__.py examples/export_stella_mode_structure_fixture.py tests/test_benchmark_references.py`
  - `uv run pytest tests/test_benchmark_references.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `rg -n "Overfull" main.log`
  - `git diff --check`
- Verification:
  - stella CMake configure and CPU build succeeded,
  - stella W7-X single-mode CPU smoke run completed,
  - stella fixture self-comparison passed with zero growth, frequency, and
    profile error,
  - focused ruff check passed,
  - benchmark-reference tests passed with `76 passed`,
  - `main.pdf` rebuilt with no overfull-box warnings,
  - `git diff --check` passed.

### 2026-06-11: Prepared Matched stella W7-X Production Input

- Added `scripts/prepare_stella_w7x_reference_run.py`, which writes a matched
  stella W7-X reference run directory for the external mode-structure gate.
- Generated `fixtures/stella_w7x_mode_structure_run/` with:
  - `stella_w7x_adiabatic_electrons.in`,
  - copied GX benchmark VMEC `wout_w7x.nc`,
  - `mode_structure_run_metadata.json`,
  - `run_stella_reference.sh`,
  - `export_stella_fixture.sh`,
  - README with the run/export/compare commands.
- The stella production input matches the current W7-X gate controls:
  GX benchmark VMEC hash
  `7d3bc31a4dd599b30619444da740c64fba27be997a2aaa25f5f211387616ab86`,
  `torflux=0.64`, `alpha0=0`, `nfield_periods=34.75236` computed from
  `gx_npol*q_eik*nfp`, one kinetic ion species, adiabatic-electron
  `field-line-average-term`, `ky=(0,0.1,0.2,0.3)`, `kx=0`, `nzed=256`,
  `nmu=8`, `nvgrid=16`, `tend=200`, `delt=0.1`, and late-window fraction
  `0.5`.
- Ran a reduced matched-physics stella smoke in
  `/private/tmp/stella_w7x_matched_smoke` with the same VMEC, surface,
  electron model, `ky` grid, and field-line-length formula but smaller
  resolution/time. stella reported no kinetic electrons, the adiabatic
  Boltzmann response, W7-X VMEC at `torflux=0.64`, `nky=4`, and standard
  twist-and-shift boundary conditions.
- Exported the smoke `.out.nc` to a portable fixture with
  `ky=(0.1,0.2,0.3)`, confirming the stella loader/exporter and strict `ky`
  matching. The short smoke values were damped, as expected for the nonproduction
  run: `gamma=(-5.045904750820995e-04, -1.54650924803e-03,
  -2.58295264995e-03)`.
- Commands run:
  - `uv run python scripts/prepare_stella_w7x_reference_run.py --overwrite`
  - `uv run python scripts/prepare_stella_w7x_reference_run.py --output-dir /private/tmp/stella_w7x_matched_smoke --nzed 24 --nmu 4 --nvgrid 4 --tend 1.0 --nwrite 1 --overwrite`
  - `/Users/mohsensadr/Codes/GitHub/new-plasma-code/relevant-codes/stella/stella stella_w7x_adiabatic_electrons.in`
  - `uv run python examples/export_stella_mode_structure_fixture.py --stella-output /private/tmp/stella_w7x_matched_smoke/stella_w7x_adiabatic_electrons.out.nc --ky-values 0.1,0.2,0.3 --average-fraction 0.5 --output /private/tmp/stella_w7x_matched_smoke/stella_w7x_mode_structure_fixture.csv`
  - `uv run ruff check scripts/prepare_stella_w7x_reference_run.py tests/test_stella_w7x_reference_run.py examples/export_stella_mode_structure_fixture.py src/stellarator_gk/benchmarks.py`
  - `uv run pytest tests/test_stella_w7x_reference_run.py tests/test_benchmark_references.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `rg -n "Overfull" main.log`
  - `git diff --check`
- Verification:
  - matched stella production input and metadata generated,
  - reduced matched-physics stella smoke completed,
  - smoke fixture export wrote three requested nonzonal `ky` rows,
  - focused ruff check passed,
  - focused tests passed with `79 passed`,
  - `main.pdf` rebuilt with no overfull-box warnings,
  - `git diff --check` passed.

### 2026-06-10: Generated W7-X External Handoff Bundle and Refreshed Blockers

- Ran the W7-X external-reference workflow with `--copy-vmec`, which copied
  `wout_w7x.nc` into `fixtures/gx_w7x_mode_structure_run/` and regenerated
  `external_reference_status.json`.
- Generated
  `fixtures/gx_w7x_mode_structure_run/w7x_external_reference_bundle.tar.gz`
  with seven payload files plus `BUNDLE_MANIFEST.json`: patched GX input,
  metadata, README, three handoff scripts, and the W7-X VMEC file.
- Refreshed
  `fixtures/gx_w7x_mode_structure_run/external_reference_ingest_status.json`:
  it remains `blocked_missing_external_outputs` until the GX `.big.nc` and
  `.out.nc` files are returned.
- Refreshed
  `fixtures/w7x_itg_convergence_study/production_readiness_gate.json`: the
  reduced convergence regression still passes, while production readiness
  remains `blocked_external_reference`.
- Added `local_bundle` and `local_bundle_exists` fields to
  `scripts/run_w7x_external_reference_workflow.py` so the machine-readable
  blocker report distinguishes a prepared-transfer state from an unprepared
  handoff.
- Commands run:
  - `uv run python scripts/run_w7x_external_reference_workflow.py --copy-vmec`
  ...
  - `uv run python scripts/ingest_w7x_external_reference_outputs.py --copy-outputs --resample-reference-to-observed-z`
  - `uv run python scripts/run_w7x_production_readiness_gate.py`
  - `uv run ruff check scripts/run_w7x_external_reference_workflow.py scripts/package_w7x_external_reference_bundle.py scripts/ingest_w7x_external_reference_outputs.py tests/test_w7x_external_reference_workflow.py tests/test_w7x_production_readiness_gate.py`
  - `uv run pytest tests/test_w7x_external_reference_workflow.py tests/test_w7x_production_readiness_gate.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `git diff --check`
  - `rg -n "Overfull" main.log`
- Verification:
  - external workflow status is `blocked_missing_gx_executable`,
  - local bundle exists and is recorded in the status JSON,
  - ingest status is `blocked_missing_external_outputs`,
  - production readiness is `blocked_external_reference`,
  - focused ruff check passed,
  - focused W7-X external/readiness tests passed with `10 passed`,
  - `main.tex` rebuilt to a 34-page PDF,
  - whitespace check passed and no overfull boxes were reported.

### 2026-06-10: Consolidated Remaining Stellarator/Turbulence Roadmap

- Re-read `task.tex` and the current implementation state to separate three
  milestones:
  - reduced linear stellarator scans, which are executable today,
  - trusted externally validated linear W7-X stellarator runs, which are
    blocked by missing external `.big.nc`/`.out.nc` reference outputs and
    production convergence/timing,
  - full nonlinear turbulence stellarator runs, which require later nonlinear
    ExB/dealiasing, flux diagnostics, and nonlinear benchmark parity.
- Updated `TODO.md` so the task-roadmap alignment and Immediate Next Round
  explicitly list the remaining discrepancies: missing W7-X external reference,
  nonconverged reduced W7-X ladder, GKW full-state/velocity-slice parity gap,
  exact-grid low-`ky` Cyclone branch-shape gap, reduced Hermite-Laguerre
  backend status, reduced DESC optimization status, and deferred nonlinear
  physics.
- Updated `STATUS.md` with a `task.tex` status summary and a refreshed next
  implementation round ordered around W7-X external parity, production-shape
  gate, production convergence, CPU timing, and DESC readiness.
- Updated `main.tex` with a concise result-section subsection on what remains
  for a full stellarator run and which discrepancies still need resolution.
- Commands run:
  - `uv run ruff check scripts/run_w7x_external_reference_workflow.py scripts/package_w7x_external_reference_bundle.py scripts/ingest_w7x_external_reference_outputs.py tests/test_w7x_external_reference_workflow.py tests/test_w7x_production_readiness_gate.py`
  - `uv run pytest tests/test_w7x_external_reference_workflow.py tests/test_w7x_production_readiness_gate.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
...
### 2026-06-09: Hardened W7-X External Return/Ingest Blocker Path

- Added `scripts/ingest_w7x_external_reference_outputs.py`, a repository-side
  return-path driver for externally generated W7-X GX outputs. It accepts or
  copies returned `.big.nc`/`.out.nc` files, exports
  `fixtures/w7x_itg_external_mode_structure_fixture.csv`, runs the W7-X
  mode-structure gate, and refreshes the production-readiness ledger.
- Added `fixtures/gx_w7x_mode_structure_run/ingest_returned_outputs.sh` as the
  checked-in shell handoff for the returned-output path.
- Extended `scripts/run_w7x_external_reference_workflow.py` and
  `external_reference_status.json` with:
  - a returned-output ingest command,
  - local capability audit fields for `gx`, `nvcc`, `nvidia-smi`, and the GX
    source tree,
  - an explicit action to return `.big.nc`/`.out.nc` outputs and run the ingest
    script.
- Extended `scripts/package_w7x_external_reference_bundle.py` so the bundle
  manifest records that it should be unpacked over a full repository checkout,
  includes the returned-output ingest command, and packages
  `ingest_returned_outputs.sh`.
- Added
  `fixtures/gx_w7x_mode_structure_run/external_reference_ingest_status.json`.
  Current status is `blocked_missing_external_outputs`: the local repository is
  ready to ingest returned GX files, but the matched W7-X `.big.nc`/`.out.nc`
  pair is still absent.
- Updated `TODO.md`, `STATUS.md`, `main.tex`, and the W7-X external-run README
  with the package/run/ingest/timing command sequence.
- Added tests covering the local capability report, returned-output ingest
  command, bundle manifest, handoff scripts, missing returned outputs, and the
  copied-output ingest orchestration path.
- Commands run:
  ...
  - `uv run python scripts/package_w7x_external_reference_bundle.py --output /tmp/w7x_external_reference_bundle.tar.gz`
  - `uv run ruff check scripts/run_w7x_external_reference_workflow.py scripts/package_w7x_external_reference_bundle.py scripts/ingest_w7x_external_reference_outputs.py tests/test_w7x_external_reference_workflow.py`
  ...
  - `rg -n "Overfull" main.log`
- Verification:
  - external workflow status regenerated as `blocked_missing_gx_executable`,
  - ingest status generated as `blocked_missing_external_outputs`,
  - transfer bundle wrote to `/tmp/w7x_external_reference_bundle.tar.gz` with
    7 files,
  - focused ruff check passed,
  - external workflow/ingest tests passed with `8 passed`,
  - W7-X production readiness remains `blocked_external_reference`,
  - `main.tex` rebuilt to a 34-page PDF,
  - whitespace check passed and no overfull boxes were reported.
...
- Added shell handoff scripts in `fixtures/gx_w7x_mode_structure_run/`:
  - `run_external_reference.sh` runs the external GX workflow using
    `GX_EXECUTABLE` and refreshes the production-readiness ledger,
  - `run_production_timing_after_parity.sh` reruns the guarded production CPU
    timing and then requires the readiness gate to pass.
- Added `scripts/package_w7x_external_reference_bundle.py`, which writes a
  portable tarball containing the patched GX input, VMEC source, README,
  handoff scripts, and `BUNDLE_MANIFEST.json` with file sizes and SHA-256
  hashes for transfer to the GX/CUDA machine.
- Generated
...
- Added `tests/test_w7x_external_reference_workflow.py` covering missing-GX,
  VMEC-copy, already-exported-fixture states, committed handoff scripts, and
  the transfer-bundle manifest.
- Commands run:
  - `uv run python scripts/run_w7x_external_reference_workflow.py`
  - `uv run python scripts/package_w7x_external_reference_bundle.py --output /tmp/w7x_external_reference_bundle.tar.gz`
  - `uv run ruff check scripts/run_w7x_external_reference_workflow.py scripts/package_w7x_external_reference_bundle.py tests/test_w7x_external_reference_workflow.py`
  - `uv run pytest tests/test_w7x_external_reference_workflow.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `git diff --check`
  - `rg -n "Overfull" main.log`
- Verification:
  ...
  - transfer bundle wrote to `/tmp/w7x_external_reference_bundle.tar.gz`,
  - focused ruff check passed,
  - external workflow tests passed with `6 passed`.
  - `main.tex` rebuilt to a 34-page PDF with no overfull boxes.

### 2026-06-08: Completed Item 5 Implementation Layer

- Added `scripts/run_w7x_production_cpu_timing.py`, a guarded CPU-timing
  artifact writer for W7-X production-control dimensions. By default it writes
  `fixtures/w7x_itg_convergence_study/production_cpu_timing.json` with
  `status="blocked_until_external_parity_passes"` and no production timing
  claim. With `--allow-pending-external-parity`, it can run a development or
  smoke residual timing path; after external parity passes it becomes the true
  production CPU timing writer.
- Updated `scripts/run_w7x_production_readiness_gate.py` so the ledger records
  the presence and status of the CPU-timing artifact while still refusing to
  pass production timing before external parity passes.
- Updated `examples/desc_fixture_optimization_loop.py` so the reduced DESC
  optimization demo prints the production-readiness status and refuses
  `--require-production-ready` while the W7-X readiness gate is open.
- Added `tests/test_w7x_production_cpu_timing.py` and
  `tests/test_desc_optimization_readiness_gate.py` for the blocked production
  contract, smoke timing override, missing-readiness status, and production
  optimization guard.
- Updated `TODO.md` to mark item 5 implementation complete and separate the
  remaining external production-claim blockers: exporting the matched W7-X
  external mode-structure fixture and rerunning true production CPU timing
  after external parity passes.
- Commands run:
  - `uv run python scripts/run_w7x_production_cpu_timing.py`
  - `uv run python scripts/run_w7x_production_readiness_gate.py`
  - `uv run ruff check scripts/run_w7x_production_cpu_timing.py scripts/run_w7x_production_readiness_gate.py scripts/run_w7x_reduced_convergence_study.py examples/desc_fixture_optimization_loop.py tests/test_w7x_production_cpu_timing.py tests/test_w7x_production_readiness_gate.py tests/test_w7x_convergence_study.py tests/test_desc_optimization_readiness_gate.py`
  - `uv run pytest tests/test_w7x_production_cpu_timing.py tests/test_w7x_production_readiness_gate.py tests/test_w7x_convergence_study.py tests/test_w7x_mode_structure_gate.py tests/test_desc_optimization_readiness_gate.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `git diff --check`
- Verification:
  - CPU-timing contract artifact generated with
    `status="blocked_until_external_parity_passes"`,
  - readiness ledger regenerated with reduced convergence passing and
    production readiness still blocked by the external reference,
  - focused ruff check passed,
  - focused W7-X/optimization-readiness tests passed with `11 passed`,
  - `main.tex` rebuilt and `main.log` has no overfull boxes,
  - whitespace check passed.

### 2026-06-08: Added W7-X Production Readiness Gate

- Added `scripts/run_w7x_production_readiness_gate.py`, a one-command ledger
  for item 5. It audits the committed reduced W7-X convergence/timing
  artifacts, runs the W7-X external mode-structure gate, checks for a future
  true production CPU timing artifact, and records whether DESC optimization
  can move beyond the reduced label.
- Added `tests/test_w7x_production_readiness_gate.py` covering both current
  blocked states:
  - missing external W7-X mode-structure reference,
  - external self-reference passing while production CPU timing remains absent.
- Generated
  `fixtures/w7x_itg_convergence_study/production_readiness_gate.json` and
  `fixtures/w7x_itg_convergence_study/external_mode_structure_gate/gate_status.json`.
  The reduced convergence regression passes; the production gate is currently
  `blocked_external_reference`.
- Commands run:
  - `uv run python scripts/run_w7x_production_readiness_gate.py`
  - `uv run ruff check scripts/run_w7x_production_readiness_gate.py scripts/run_w7x_reduced_convergence_study.py tests/test_w7x_production_readiness_gate.py tests/test_w7x_convergence_study.py tests/test_w7x_mode_structure_gate.py`
  - `uv run pytest tests/test_w7x_production_readiness_gate.py tests/test_w7x_convergence_study.py tests/test_w7x_mode_structure_gate.py -q`
  - `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex`
  - `git diff --check`
- Verification:
  - production-readiness ledger generated with `passed=false`,
  - focused ruff check passed,
  - focused W7-X convergence/readiness/gate tests passed with `7 passed`,
  - `main.tex` rebuilt and `main.log` has no overfull boxes,
  - whitespace check passed.
