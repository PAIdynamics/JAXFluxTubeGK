submodule (diag_files_m) checkpoint_file_s
    use mpi
...
contains
    ...
    integer module function calculate_process_id(this)
        ...
        calculate_process_id = coords(1) + 1
        do i = 2, 5
            calculate_process_id = calculate_process_id &
    ...
    module subroutine initialize_checkpoint(this, dcomm_handler, &
        ...
        integer, intent(in) :: n_points_RZ, n_points_phi, n_points_vp, &
                               n_points_mu, n_points_sp
        logical, optional, intent(in) :: from_existing

        integer :: n, ierr, process_id, dataset_dims(6)
        ...
        this%filename = remove_filename_ending(filename) // "_" &
                        // string(this%calculate_process_id()) // ".nc"

        if (from_existing_loc) then
            ! Open existing checkpoint_file for read and write access
            ...
            ierr = nf90_inq_varid(this%file_id, 'dist_func', this%dist_func_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ...
            ierr = nf90_def_dim(this%file_id, 'dim_RZ', &
                                n_points_RZ / dcomm_handler%get_n_procs_RZ(), &
                                this%dim_RZ_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ...
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ! Define vp dimension
            ierr = nf90_def_dim(this%file_id, 'dim_vp', &
                                n_points_vp / dcomm_handler%get_n_procs_vp(), &
                                this%dim_vp_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ! Define mu dimension
            ierr = nf90_def_dim(this%file_id, 'dim_mu', &
                                n_points_mu / dcomm_handler%get_n_procs_mu(), &
                                this%dim_mu_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ! Define sp dimension
            ierr = nf90_def_dim(this%file_id, 'dim_sp', &
                                n_points_sp / dcomm_handler%get_n_procs_sp(), &
                                this%dim_sp_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)

            ! Define boundaries dimension
            ierr = nf90_def_dim(this%file_id, 'dim_bound', 5, &
                                this%dim_bound_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
            ...
            dataset_dims(3) = this%dim_vp_id
            dataset_dims(4) = this%dim_mu_id
            dataset_dims(5) = this%dim_sp_id
            dataset_dims(6) = this%dim_time_id

            ierr = nf90_def_var(this%file_id, 'dist_func', NF90_GP, &
                                dataset_dims, this%dist_func_id)
            call handle_error_netcdf(ierr, __LINE__, __FILE__)
    ...
    module subroutine write_checkpoint(this, time, dist_func)
        ...
        integer :: ierr
        ! NetCDF error status
        integer :: start(6), start_time(1)
        ! NetCDF index arrays where the first of the data values be written
        integer, dimension(5) :: lb_stripped, ub_stripped
        ! The lower and upper boundaries of the distribution function

        ! Get the lower and upper boundaries from dist_func
        lb_stripped = lbound(dist_func)
        ub_stripped = ubound(dist_func)

        if (this%write_counter .eq. 1) then
        ...
        start = [1, 1, 1, 1, 1, start_time(1)]

        ! Write distribution function
        ierr = nf90_put_var(this%file_id, this%dist_func_id, dist_func, &
                            start = start)
        call handle_error_netcdf(ierr, __LINE__, __FILE__)
    ...
    module subroutine read_checkpoint(this, time, dist_func)
        ...
        real(kind = GP), pointer, &
                         dimension(:, :, :, :, :), intent(inout) :: dist_func

        integer :: ierr
        ! NetCDF error status
        integer :: start(6), start_time(1)
        ! NetCDF index arrays where the first of the data values be read
        integer, dimension(5) :: lb_stripped_file, ub_stripped_file
        ! The lower and upper boundaries read from checkpoint files
        integer, dimension(5) :: lb_stripped_ref, ub_stripped_ref
        ! The lower and upper boundaries obtained from dist_func as reference
        ...
        ierr = nf90_get_var(this%file_id, this%dist_func_id, dist_func, &
                            start = start)
        call handle_error_netcdf(ierr, __LINE__, __FILE__)

        ! Get the lower and upper boundaries from dist_func for reference
        lb_stripped_ref = lbound(dist_func)
        ub_stripped_ref = ubound(dist_func)
